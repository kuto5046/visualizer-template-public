...
use svg::node::element::{Circle, Polygon, Rectangle, Title, SVG};
...
const RATIO: f64 = H / COORD_MAX as f64;
...
pub struct Output {
    ...
    initial_a: Vec<usize>,
    pub commented_ops: Vec<CommentedOp>,
}

pub fn parse_output(s: &str, input: &Input) -> Result<Output> {
...
fn parse_op(line: &str, input: &Input) -> Result<Op> {
    ...
    match type_ {
        's' => {
            let len: usize = read(tokens.next(), 0, input.n - 1).context("l")?;
...
impl State {
    fn new(input: &Input, output: &Output) -> State {
        let mut total_visited = vec![false; input.n];
        total_visited[0] = true;
        State {
            score: 0,
            op_cnt: 0,
            target_idx: 0,
...
pub fn draw_graph(vis_data: &VisData, input: &Input, mut doc: SVG) -> SVG {
    ...
    for (i, &(x, y)) in input.coordinates.iter().enumerate() {
        if vis_data.state.traffic_light[i] == 0 {
            let col = if vis_data.state.total_visited[i] {
                "gray"
            } else {
                "orangered"
            };
