...
pub fn vis(input: &Input, output: &Output, turn: usize) -> (i64, String, String) {
    let H = BOX_SIZE * input.H as f32;
    let W = BOX_SIZE * input.W as f32;


    // k作物種,i,j: 座標,s: 植える時期
    let mut plant_map = mat![-1; input.H; input.W];   // 植えてる時に緑にする
    for w in &output.works {
        // D[w.k]: 作物kの収穫時期
        // S[w.k]: 作物kの植える時期
        // 植える前の作物は無視, 収穫時期の作物も無視
        if turn < w.s || input.D[w.k] < turn {
            continue;
        }
        // havest_map[w.i][w.j] = input.D[w.k] - input.S[w.k] + 1;
        let remain_month = input.D[w.k] - turn;
        plant_map[w.i][w.j] = remain_month as i32;
    }
    ...
    for i in 0..input.H {
        for j in 0..input.W {
            let remain_month = plant_map[i][j];
            let color = if remain_month == 0 {
                "Red"
            ...
            if remain_month != -1 {
                let text = SvgText::new()
                    ...
                    .add(TextContent::new(format!("{}", remain_month)));
                doc = doc.add(text);
            }
        }
    }
