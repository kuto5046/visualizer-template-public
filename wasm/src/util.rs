use proconio::{input, marker::Usize1};
use rand::prelude::*;
use rand_chacha::ChaCha20Rng;
use rand_distr::Normal;
use itertools::Itertools;
use std::collections::HashSet;
use svg::node::{
    element::{Circle, Group, Line, Rectangle, Title, Text},
    Text as TextContent,
};
// document
use svg::Document;



pub trait SetMinMax {
    fn setmin(&mut self, v: Self) -> bool;
    fn setmax(&mut self, v: Self) -> bool;
}
impl<T> SetMinMax for T
where
    T: PartialOrd,
{
    fn setmin(&mut self, v: T) -> bool {
        *self > v && {
            *self = v;
            true
        }
    }
    fn setmax(&mut self, v: T) -> bool {
        *self < v && {
            *self = v;
            true
        }
    }
}

pub const MAP_SIZE: i32 = 10000;
pub const MAP_SIZE2: i32 = MAP_SIZE * 2;
pub const MAX_POWER: i32 = 5000;
pub const MIN_BASE_COUNT: i32 = 100;
pub const MAX_BASE_COUNT: i32 = 100;
pub const MIN_BASE_DIST: i32 = 1000;
pub const MIN_HOUSE_COUNT: i32 = 2000;
pub const MAX_HOUSE_COUNT: i32 = 5000;
pub const MIN_PIVOT_COUNT: i32 = 20;
pub const MAX_PIVOT_COUNT: i32 = 40;
pub const MIN_PIVOT_DIST: i32 = 2000;
pub const PIVOT_MAP_SIZE: i32 = 8000;
pub const MIN_STD_DEV: f64 = 200.0;
pub const MAX_STD_DEV: f64 = 1000.0;
pub const MIN_EDGE_COST_COEF: f64 = 10.0;
pub const MAX_EDGE_COST_COEF: f64 = 50.0;

#[derive(Clone, Debug)]
pub struct Input {
    pub n: usize,
    pub m: usize,
    pub k: usize,
    pub stations: Vec<Point>,
    pub edges: Vec<(usize, usize, i32)>,
    pub residents: Vec<Point>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Output {
    pub powers: Vec<i32>,
    pub edges: Vec<bool>,
}

impl Output {
    fn get_broadcasted_count(&self, input: &Input) -> usize {
        let is_connected = self.get_connection_status(input);
        self.get_broadcasted_status(input, &is_connected)
            .iter()
            .filter(|&&b| b)
            .count()
    }

    fn get_connection_status(&self, input: &Input) -> Vec<bool> {
        let mut dsu = Dsu::new(input.n);

        for (j, used) in self.edges.iter().enumerate() {
            if !used {
                continue;
            }

            let (u, v, _) = input.edges[j];
            dsu.merge(u, v);
        }

        (0..input.n).map(|i| dsu.same(0, i)).collect()
    }

    fn get_broadcasted_status(&self, input: &Input, is_connected: &[bool]) -> Vec<bool> {
        let mut broadcasted = vec![false; input.k];

        for i in 0..input.n {
            if !is_connected[i] {
                continue;
            }

            for k in 0..input.k {
                let dist_sq = input.stations[i].calc_sq_dist(&input.residents[k]);
                let power = self.powers[i];
                broadcasted[k] |= dist_sq <= power * power;
            }
        }

        broadcasted
    }

    fn calc_cost(&self, input: &Input) -> i64 {
        let mut cost = 0;

        for i in 0..input.n {
            cost += self.calc_power_cost(i);
        }

        for (j, used) in self.edges.iter().enumerate() {
            if !used {
                continue;
            }

            let (_, _, w) = input.edges[j];
            cost += w as i64;
        }

        cost
    }

    fn calc_power_cost(&self, v: usize) -> i64 {
        let p = self.powers[v] as i64;
        p * p
    }
}

#[derive(Clone, Debug, Copy, PartialEq, Eq, Hash)]
pub struct Point {
    pub x: i32,
    pub y: i32,
}

impl Point {
    pub const fn new(x: i32, y: i32) -> Self {
        Self { x, y }
    }

    pub fn calc_sq_dist(&self, other: &Point) -> i32 {
        let dx = self.x - other.x;
        let dy = self.y - other.y;
        dx * dx + dy * dy
    }
}

impl std::fmt::Display for Input {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "{} {} {}", self.n, self.m, self.k)?;
        for p in &self.stations {
            writeln!(f, "{} {}", p.x, p.y)?;
        }

        for &(u, v, w) in &self.edges {
            writeln!(f, "{} {} {}", u + 1, v + 1, w)?;
        }

        for p in &self.residents {
            writeln!(f, "{} {}", p.x, p.y)?;
        }

        Ok(())
    }
}

pub fn parse_input(f: &str) -> Input {
    let f = proconio::source::once::OnceSource::from(f);
    input! {
        from f,
        n: usize,
        m: usize,
        k: usize,
        stations: [(i32, i32); n],
        edges: [(Usize1, Usize1, i32); m],
        residents: [(i32, i32); k],
    }

    Input {
        n,
        m,
        k,
        stations: stations.iter().map(|&(x, y)| Point::new(x, y)).collect(),
        edges,
        residents: residents.iter().map(|&(x, y)| Point::new(x, y)).collect(),
    }
}

fn read<T: Copy + PartialOrd + std::fmt::Display + std::str::FromStr>(
    token: Option<&str>,
    lb: T,
    ub: T,
) -> Result<T, String> {
    if let Some(v) = token {
        if let Ok(v) = v.parse::<T>() {
            if v < lb || ub < v {
                Err(format!("Out of range: {}", v))
            } else {
                Ok(v)
            }
        } else {
            Err(format!("Parse error: {}", v))
        }
    } else {
        Err("Unexpected EOF".to_owned())
    }
}

pub fn parse_output(input: &Input, f: &str) -> Result<Vec<Output>, String> {
    let mut outputs = vec![];
    let mut tokens = f.split_whitespace().peekable();

    while tokens.peek().is_some() {
        let mut powers = vec![];

        for _ in 0..input.n {
            powers.push(read(tokens.next(), 0, MAX_POWER)?);
        }

        let mut edges = vec![];

        for _ in 0..input.m {
            let b = read(tokens.next(), 0, 1)?;
            edges.push(b == 1);
        }

        let output = Output { powers, edges };
        outputs.push(output);
    }

    Ok(outputs)
}

pub fn compute_score(input: &Input, out: &Output) -> (i64, String) {
    let broadcasted_count = out.get_broadcasted_count(&input);

    let score = if broadcasted_count < input.k {
        (1e6 * (broadcasted_count + 1) as f64 / input.k as f64).round() as i64
    } else {
        let cost = out.calc_cost(input);
        (1e6 * (1.0 + 1e8 / (cost as f64 + 1e7))).round() as i64
    };

    (score, String::new())
}

pub fn gen(seed: u64) -> Input {
    if seed == 0 {
        return gen_seed0();
    }

    let mut rng = rand_chacha::ChaCha20Rng::seed_from_u64(seed);

    loop {
        let mut points_set = HashSet::new();
        let stations = gen_stations(&mut rng, &mut points_set);
        let n = stations.len();

        let edges = gen_edges(&stations, &mut rng);
        let m = edges.len();

        let houses = gen_residents(&mut rng, points_set);
        let k = houses.len();

        if check_reachable(&stations, &houses) {
            return Input {
                n,
                m,
                k,
                residents: houses,
                stations,
                edges,
            };
        }
    }
}

pub fn gen_stations(rng: &mut ChaCha20Rng, points_set: &mut HashSet<Point>) -> Vec<Point> {
    let n = rng.gen_range(MIN_BASE_COUNT, MAX_BASE_COUNT + 1);
    let mut stations = vec![];
    stations.push(Point::new(0, 0));

    for _ in 1..n {
        loop {
            let x = rng.gen_range(-MAP_SIZE, MAP_SIZE + 1);
            let y = rng.gen_range(-MAP_SIZE, MAP_SIZE + 1);

            let point = Point::new(x, y);

            if stations
                .iter()
                .all(|p| p.calc_sq_dist(&point) >= MIN_BASE_DIST * MIN_BASE_DIST)
                && points_set.insert(point)
            {
                stations.push(point);
                break;
            }
        }
    }
    stations
}

pub fn gen_edges(points: &[Point], rng: &mut ChaCha20Rng) -> Vec<(usize, usize, i32)> {
    let points_f64 = points
        .iter()
        .map(|p| delaunator::Point {
            x: p.x as f64,
            y: p.y as f64,
        })
        .collect_vec();
    let triangles = delaunator::triangulate(&points_f64).triangles;
    assert_eq!(triangles.len() % 3, 0);

    let mut edges = vec![];

    for triangle in triangles.chunks(3) {
        for i in 0..3 {
            let u = triangle[i];
            let v = triangle[(i + 1) % 3];
            assert_ne!(u, v);
            edges.push((u.min(v), u.max(v)));
        }
    }

    edges.sort_unstable();
    edges.dedup();

    edges
        .iter()
        .map(|&(u, v)| {
            let dist = (points[u].calc_sq_dist(&points[v]) as f64).sqrt().round();
            let coef = rng.gen_range(MIN_EDGE_COST_COEF, MAX_EDGE_COST_COEF);
            let weight = (coef * coef * dist).round() as i32;
            assert!(100 * dist as i32 <= weight && weight <= 2500 * dist as i32);
            (u, v, weight)
        })
        .collect_vec()
}

pub fn gen_residents(rng: &mut ChaCha20Rng, mut points_set: HashSet<Point>) -> Vec<Point> {
    let k = rng.gen_range(MIN_HOUSE_COUNT, MAX_HOUSE_COUNT + 1);
    let pivot_count = rng.gen_range(MIN_PIVOT_COUNT, MAX_PIVOT_COUNT + 1);
    let mut pivot_centers: Vec<Point> = vec![];

    for _ in 0..pivot_count {
        loop {
            let x = rng.gen_range(-PIVOT_MAP_SIZE, PIVOT_MAP_SIZE + 1);
            let y = rng.gen_range(-PIVOT_MAP_SIZE, PIVOT_MAP_SIZE + 1);

            let pivot = Point::new(x, y);

            if pivot_centers
                .iter()
                .all(|p| p.calc_sq_dist(&pivot) >= MIN_PIVOT_DIST * MIN_PIVOT_DIST)
            {
                pivot_centers.push(pivot);
                break;
            }
        }
    }

    let mut pivot_stddevs = vec![];

    for _ in 0..pivot_count {
        let dev = rng.gen_range(MIN_STD_DEV, MAX_STD_DEV);
        pivot_stddevs.push(dev);
    }

    let mut houses = vec![];

    for _ in 0..k {
        loop {
            let r = rng.gen_range(0, pivot_count) as usize;
            let pivot = pivot_centers[r];
            let std_dev = pivot_stddevs[r];
            let x = Normal::new(pivot.x as f64, std_dev)
                .unwrap()
                .sample(rng)
                .round();
            let y = Normal::new(pivot.y as f64, std_dev)
                .unwrap()
                .sample(rng)
                .round();

            if x.abs() > 1e9 || y.abs() > 1e9 {
                continue;
            }

            let p = Point::new(x as i32, y as i32);

            if p.x.abs() <= MAP_SIZE && p.y.abs() <= MAP_SIZE && points_set.insert(p) {
                houses.push(p);
                break;
            }
        }
    }

    houses
}

pub fn check_reachable(stations: &[Point], residents: &[Point]) -> bool {
    let mut remaining = residents.iter().collect_vec();

    for &station in stations {
        remaining.retain(|p| p.calc_sq_dist(&station) > MAX_POWER * MAX_POWER);
    }

    remaining.len() == 0
}

pub struct Dsu {
    n: usize,
    parent_or_size: Vec<i32>,
}

impl Dsu {
    pub fn new(size: usize) -> Self {
        Self {
            n: size,
            parent_or_size: vec![-1; size],
        }
    }

    pub fn merge(&mut self, a: usize, b: usize) -> usize {
        assert!(a < self.n);
        assert!(b < self.n);
        let (mut x, mut y) = (self.leader(a), self.leader(b));
        if x == y {
            return x;
        }
        if -self.parent_or_size[x] < -self.parent_or_size[y] {
            std::mem::swap(&mut x, &mut y);
        }
        self.parent_or_size[x] += self.parent_or_size[y];
        self.parent_or_size[y] = x as i32;
        x
    }

    pub fn same(&mut self, a: usize, b: usize) -> bool {
        assert!(a < self.n);
        assert!(b < self.n);
        self.leader(a) == self.leader(b)
    }

    pub fn leader(&mut self, a: usize) -> usize {
        assert!(a < self.n);
        if self.parent_or_size[a] < 0 {
            return a;
        }
        self.parent_or_size[a] = self.leader(self.parent_or_size[a] as usize) as i32;
        self.parent_or_size[a] as usize
    }

    pub fn size(&mut self, a: usize) -> usize {
        assert!(a < self.n);
        let x = self.leader(a);
        -self.parent_or_size[x] as usize
    }

    pub fn groups(&mut self) -> Vec<Vec<usize>> {
        let mut leader_buf = vec![0; self.n];
        let mut group_size = vec![0; self.n];
        for i in 0..self.n {
            leader_buf[i] = self.leader(i);
            group_size[leader_buf[i]] += 1;
        }
        let mut result = vec![Vec::new(); self.n];
        for i in 0..self.n {
            result[i].reserve(group_size[i]);
        }
        for i in 0..self.n {
            result[leader_buf[i]].push(i);
        }
        result
            .into_iter()
            .filter(|x| !x.is_empty())
            .collect::<Vec<Vec<usize>>>()
    }
}

pub fn group(title: String) -> Group {
    // Textはnode::element::Textもあるので注意。これによってtooltipが表示されなかったかもしれない
    Group::new().add(Title::new().add(TextContent::new(title)))
}


pub fn vis(input: &Input, output: &Output, turn: usize) -> (i64, String, String) {
    // まず枠を作る
    let W = 800;
    let is_connected = output.get_connection_status(input);
    let is_broadcasted = output.get_broadcasted_status(input, &is_connected);
    let mut doc = Document::new()
                        .set("id", "vis")
                        .set("viewBox", (-10, -10, W + 20, W + 20))
                        .set("width", W)
                        .set("height", W);
                        //.set("style", "background-color:white");

    let rect = Rectangle::new().set("x", -10)
                              .set("y", -10)
                              .set("width", W+20)
                              .set("height", W+20)
                              .set("fill", "White");
    doc = doc.add(rect);

    // residentsの点を描画する
    for (i, p) in input.residents.iter().enumerate() {
        let cx = W * (p.x + MAP_SIZE) / MAP_SIZE2;
        let cy = W - W * (p.y + MAP_SIZE) / MAP_SIZE2;
        let color = if is_broadcasted[i] { "LightCoral" } else { "White" };
        let circle = Circle::new().set("cx", cx)
                                  .set("cy", cy)
                                  .set("r", 3)
                                  .set("fill", color)
                                  .set("fill-opacity", 0.7)
                                  .set("stroke", "Gray")
                                  .set("stroke-width", 1);
        let mut gr = group(format!("resident={} \n(x,y)=({},{})", i, p.x, p.y));
        gr = gr.add(circle);
        doc = doc.add(gr);
    }

    // stationsの点を描画する
    for (i, p) in input.stations.iter().enumerate() {
        let cx = W * (p.x + MAP_SIZE) / MAP_SIZE2;
        let cy = W - W * (p.y + MAP_SIZE) / MAP_SIZE2;
        let power = output.powers[i];
        let (fill_color, stroke_color, stroke_width) = if power > 0 { ("LightBlue", "Gray", 2)} else { ("White", "LightGray", 1) };
        let r = W * output.powers[i] / MAP_SIZE2;
        let circle = Circle::new().set("cx", cx)
                                  .set("cy", cy)
                                  .set("r", r)
                                  .set("fill", "LightBlue")
                                  .set("fill-opacity", 0.3);
        let rect = Rectangle::new().set("x", cx)
                                  .set("y", cy)
                                  .set("width", 8)
                                  .set("height", 8)
                                  .set("fill", fill_color)
                                  .set("stroke", stroke_color)
                                  .set("stroke-width", stroke_width);
        let mut gr = group(format!("station={} \n(x,y)=({},{})\n power={}", i, p.x, p.y, power));
        // MEMO: rectとcircleを両方同じgroupに入れる。別にしてるとtooltipが表示されない
        gr = gr.add(rect).add(circle);
        doc = doc.add(gr);
    }


    // stationsの間を線で結ぶ
    for (i, (u, v, w)) in input.edges.iter().enumerate() {
        let u_station = &input.stations[*u];
        let v_station = &input.stations[*v];
        let edge_on_off = output.edges[i];
        let (color, sw) = if edge_on_off { ("Gray", 2) } else { ("LightGray", 1) };
        doc = doc.add(
            Line::new()
            .set("x1", W * (u_station.x + MAP_SIZE) / MAP_SIZE2)
            .set("y1", W - W * (u_station.y + MAP_SIZE) / MAP_SIZE2)
            .set("x2", W * (v_station.x + MAP_SIZE) / MAP_SIZE2)
            .set("y2", W - W * (v_station.y + MAP_SIZE) / MAP_SIZE2)
            .set("stroke", color)
            .set("stroke-width", sw)
        )
    }

    // outputで頂点iの強度に応じてrを大きくする


    (0, "".to_string(), doc.to_string())
}


pub fn gen_seed0() -> Input {
    let n = 100;
    let m = 284;
    let k = 5000;
    let stations = vec![
        Point::new(0, 0),
        Point::new(957, -6517),
        Point::new(-412, -5500),
        Point::new(-299, 4489),
        Point::new(-8357, 6534),
        Point::new(1974, -6619),
        Point::new(1410, -8691),
        Point::new(8833, -9829),
        Point::new(7785, 2682),
        Point::new(-9839, 443),
        Point::new(-5084, 6063),
        Point::new(8064, 4821),
        Point::new(7008, 1946),
        Point::new(2894, -5980),
        Point::new(-3445, 9927),
        Point::new(-1403, 3591),
        Point::new(5030, 5758),
        Point::new(3178, 1199),
        Point::new(-9263, -3926),
        Point::new(-3126, -2879),
        Point::new(4179, 4360),
        Point::new(5715, 595),
        Point::new(7209, -8569),
        Point::new(823, 1184),
        Point::new(-746, 749),
        Point::new(-3409, 7622),
        Point::new(5468, -6147),
        Point::new(-7479, 1331),
        Point::new(1299, 8707),
        Point::new(-9525, 5693),
        Point::new(6952, -1036),
        Point::new(-1847, 7832),
        Point::new(-8129, -632),
        Point::new(2575, -8725),
        Point::new(4839, -8823),
        Point::new(-1227, -2757),
        Point::new(-865, 9847),
        Point::new(9455, -8941),
        Point::new(2105, 1380),
        Point::new(-8588, 675),
        Point::new(-3585, -8061),
        Point::new(-543, 2285),
        Point::new(7669, -4769),
        Point::new(3977, 7810),
        Point::new(-6819, 7984),
        Point::new(4477, -3530),
        Point::new(279, -3858),
        Point::new(-3531, 6521),
        Point::new(6752, 7090),
        Point::new(-3589, 3467),
        Point::new(8840, 9058),
        Point::new(1138, 4829),
        Point::new(5927, 9169),
        Point::new(-7256, -8695),
        Point::new(-5914, 5270),
        Point::new(7778, -9846),
        Point::new(-283, 7559),
        Point::new(6475, 3086),
        Point::new(6568, -5629),
        Point::new(1368, 3290),
        Point::new(-2612, -6222),
        Point::new(5559, 4602),
        Point::new(-4431, 8668),
        Point::new(-2072, 4991),
        Point::new(-6171, 666),
        Point::new(1183, -2097),
        Point::new(425, 6606),
        Point::new(-7755, -7400),
        Point::new(-4505, -5471),
        Point::new(-5776, -1337),
        Point::new(3414, 2515),
        Point::new(-9779, 7048),
        Point::new(-9781, -9234),
        Point::new(3446, -7015),
        Point::new(-6939, 9996),
        Point::new(-8407, -5104),
        Point::new(-5700, -8465),
        Point::new(2124, 7411),
        Point::new(-7239, -1981),
        Point::new(-3340, -4625),
        Point::new(8108, 288),
        Point::new(-9886, 2218),
        Point::new(-6628, 6156),
        Point::new(-5001, 1060),
        Point::new(-1090, -8501),
        Point::new(9653, -2947),
        Point::new(7313, 8497),
        Point::new(8828, -1973),
        Point::new(-9054, -7660),
        Point::new(972, 2185),
        Point::new(-9188, 4513),
        Point::new(-1869, 1170),
        Point::new(-8734, -6359),
        Point::new(-5730, 7662),
        Point::new(-6406, 2570),
        Point::new(-605, -9813),
        Point::new(8601, 5913),
        Point::new(-4170, -2869),
        Point::new(9790, 4013),
        Point::new(-5790, -5997),
    ];
    let edges = vec![
        (0, 23, 1961044),
        (0, 24, 803653),
        (0, 35, 2149178),
        (0, 38, 720570),
        (0, 65, 2666245),
        (1, 2, 198863),
        (1, 5, 981819),
        (1, 6, 1002029),
        (1, 13, 1390847),
        (1, 46, 301699),
        (1, 84, 6492902),
        (2, 35, 3720981),
        (2, 46, 972093),
        (2, 60, 1755622),
        (2, 79, 3855920),
        (2, 84, 2673331),
        (3, 15, 269170),
        (3, 41, 2257144),
        (3, 51, 384550),
        (3, 59, 3282144),
        (3, 63, 1608380),
        (3, 66, 2739694),
        (4, 29, 302160),
        (4, 44, 479283),
        (4, 71, 3777471),
        (4, 82, 1793800),
        (4, 90, 4240666),
        (5, 6, 3372544),
        (5, 13, 1088387),
        (5, 33, 2853452),
        (5, 73, 1681663),
        (6, 33, 334110),
        (6, 84, 5195354),
        (6, 95, 2122187),
        (7, 22, 1612119),
        (7, 37, 796325),
        (7, 55, 2170213),
        (8, 11, 339711),
        (8, 12, 179371),
        (8, 57, 1600443),
        (8, 80, 2137212),
        (8, 98, 3223426),
        (9, 18, 2104800),
        (9, 32, 3223016),
        (9, 39, 1052534),
        (9, 72, 19940454),
        (9, 81, 276713),
        (10, 25, 5559911),
        (10, 47, 1121239),
        (10, 49, 506384),
        (10, 54, 1601136),
        (10, 82, 3304135),
        (10, 93, 385023),
        (11, 48, 1635427),
        (11, 57, 565334),
        (11, 61, 4806528),
        (11, 96, 518106),
        (11, 98, 466285),
        (12, 21, 1945433),
        (12, 57, 1109618),
        (12, 80, 3783667),
        (13, 26, 4477802),
        (13, 45, 4807662),
        (13, 46, 7935716),
        (13, 73, 869544),
        (14, 25, 1285472),
        (14, 31, 4557285),
        (14, 36, 3494866),
        (14, 62, 1443214),
        (14, 74, 6807855),
        (15, 41, 843799),
        (15, 49, 3992263),
        (15, 63, 2231942),
        (15, 91, 5838277),
        (16, 20, 2535473),
        (16, 43, 5683991),
        (16, 48, 728267),
        (16, 61, 639200),
        (16, 77, 2768275),
        (17, 21, 2407337),
        (17, 38, 1174245),
        (17, 45, 5272611),
        (17, 65, 1396847),
        (17, 70, 2170381),
        (18, 32, 5995791),
        (18, 72, 857598),
        (18, 75, 155059),
        (18, 78, 2214286),
        (18, 88, 7776467),
        (18, 92, 4595312),
        (19, 35, 2849583),
        (19, 79, 1023302),
        (19, 83, 3777074),
        (19, 91, 4863881),
        (19, 97, 1403641),
        (20, 51, 606174),
        (20, 57, 724586),
        (20, 59, 4579338),
        (20, 61, 788171),
        (20, 70, 214051),
        (20, 77, 3645910),
        (21, 30, 1140709),
        (21, 45, 1315835),
        (21, 57, 1155185),
        (21, 70, 3695671),
        (21, 80, 1015839),
        (22, 26, 306793),
        (22, 34, 1672314),
        (22, 37, 2524742),
        (22, 42, 1052692),
        (22, 55, 2914962),
        (22, 58, 2909619),
        (23, 24, 659513),
        (23, 38, 528757),
        (23, 41, 2122367),
        (23, 89, 1653434),
        (24, 35, 1576870),
        (24, 41, 3589112),
        (24, 91, 2642427),
        (25, 31, 2061585),
        (25, 47, 2330034),
        (25, 62, 1774641),
        (25, 93, 2208248),
        (26, 34, 1421525),
        (26, 45, 2563136),
        (26, 58, 234128),
        (26, 73, 1075987),
        (27, 32, 1726387),
        (27, 39, 1203167),
        (27, 64, 2435204),
        (27, 81, 3614853),
        (27, 90, 3108610),
        (27, 94, 2541293),
        (28, 36, 659862),
        (28, 43, 363301),
        (28, 52, 2203971),
        (28, 56, 618234),
        (28, 66, 2386457),
        (28, 77, 392887),
        (29, 71, 480864),
        (29, 81, 5087445),
        (29, 90, 2017021),
        (30, 42, 472273),
        (30, 45, 2357133),
        (30, 80, 3031311),
        (30, 87, 3993946),
        (31, 36, 1110037),
        (31, 47, 3124964),
        (31, 56, 450365),
        (31, 63, 3744171),
        (32, 39, 320720),
        (32, 64, 1307750),
        (32, 69, 796949),
        (32, 78, 597053),
        (33, 34, 5211179),
        (33, 73, 4086417),
        (33, 95, 4965161),
        (34, 55, 616180),
        (34, 73, 1537344),
        (34, 95, 10296120),
        (35, 46, 3639923),
        (35, 65, 3312415),
        (35, 79, 4432286),
        (35, 91, 9798847),
        (36, 50, 6294966),
        (36, 52, 13099145),
        (36, 56, 751706),
        (37, 42, 10872435),
        (37, 85, 1113751),
        (38, 59, 1402549),
        (38, 65, 4818354),
        (38, 70, 1706215),
        (38, 89, 307747),
        (39, 81, 4831351),
        (40, 60, 964684),
        (40, 68, 481023),
        (40, 76, 3573120),
        (40, 84, 374575),
        (40, 95, 1521337),
        (40, 99, 2683506),
        (41, 59, 1432260),
        (41, 89, 3034147),
        (41, 91, 1686226),
        (42, 45, 1526992),
        (42, 58, 671191),
        (42, 85, 6370295),
        (42, 87, 538619),
        (43, 48, 6065634),
        (43, 52, 4807753),
        (43, 77, 4177840),
        (44, 71, 2618880),
        (44, 74, 3972537),
        (44, 82, 304056),
        (44, 93, 2123857),
        (45, 46, 3745842),
        (45, 58, 444678),
        (45, 65, 2562672),
        (46, 65, 646770),
        (47, 49, 659372),
        (47, 63, 3058337),
        (48, 52, 4344772),
        (48, 61, 858487),
        (48, 86, 3280762),
        (48, 96, 2922149),
        (49, 54, 4031112),
        (49, 63, 3738887),
        (49, 83, 2124241),
        (49, 91, 557352),
        (49, 94, 2374059),
        (50, 52, 1147341),
        (50, 86, 2727398),
        (50, 96, 6756121),
        (50, 98, 4446056),
        (51, 59, 518027),
        (51, 66, 4493902),
        (51, 77, 6489428),
        (52, 86, 2002471),
        (53, 67, 279235),
        (53, 72, 1224859),
        (53, 76, 2377049),
        (53, 88, 1727779),
        (53, 95, 9031630),
        (54, 82, 2525675),
        (54, 90, 5090649),
        (54, 94, 1274396),
        (55, 95, 4511336),
        (56, 63, 4110717),
        (56, 66, 974227),
        (57, 61, 1928055),
        (57, 70, 3735557),
        (59, 70, 5457620),
        (59, 89, 1590347),
        (60, 68, 1753311),
        (60, 79, 1242557),
        (60, 84, 6147456),
        (62, 74, 895640),
        (62, 93, 237995),
        (63, 66, 1308487),
        (64, 69, 367910),
        (64, 83, 1007231),
        (64, 94, 2071917),
        (66, 77, 1058484),
        (67, 75, 1865709),
        (67, 76, 2241143),
        (67, 88, 666494),
        (67, 92, 862676),
        (67, 99, 2664834),
        (68, 79, 634528),
        (68, 97, 885866),
        (68, 99, 2812839),
        (69, 78, 2126522),
        (69, 83, 3340579),
        (69, 97, 4123611),
        (71, 74, 4878159),
        (71, 81, 2591727),
        (72, 81, 13001596),
        (72, 88, 3893961),
        (72, 95, 13722741),
        (74, 93, 2394029),
        (75, 78, 4446208),
        (75, 92, 1243258),
        (75, 99, 379726),
        (76, 95, 828346),
        (76, 99, 5256971),
        (78, 97, 999057),
        (78, 99, 6305125),
        (79, 97, 4507234),
        (80, 87, 667462),
        (80, 98, 3127110),
        (81, 90, 1579832),
        (82, 90, 7020945),
        (82, 93, 1442016),
        (83, 91, 3331086),
        (83, 94, 2263243),
        (83, 97, 3139166),
        (84, 95, 2367805),
        (85, 87, 2291872),
        (85, 98, 1479464),
        (86, 96, 787230),
        (87, 98, 3739897),
        (88, 92, 849855),
        (90, 94, 5642404),
        (96, 98, 816778),
        (97, 99, 4857434),
    ];
    let residents = vec![
        Point::new(0, 9050),
        Point::new(-8434, -5520),
        Point::new(8434, -5520),
        Point::new(-1310, 3740),
        Point::new(-6670, -5520),
        Point::new(4050, -5520),
        Point::new(0, -1560),
        Point::new(-4208, -8830),
        Point::new(4208, -8830),
        Point::new(-5738, -957),
        Point::new(-3044, -8175),
        Point::new(399, 4516),
        Point::new(-669, -3886),
        Point::new(5889, -3395),
        Point::new(2649, -817),
        Point::new(31, -3195),
        Point::new(-3271, -8565),
        Point::new(-7515, -4319),
        Point::new(-16, -5127),
        Point::new(6091, -4920),
        Point::new(-2785, 2290),
        Point::new(-210, -3577),
        Point::new(4279, -1060),
        Point::new(2889, -8597),
        Point::new(-1215, -3791),
        Point::new(4387, -2243),
        Point::new(2575, -2330),
        Point::new(4326, -4050),
        Point::new(-3630, 2013),
        Point::new(-423, -5827),
        Point::new(261, -4012),
        Point::new(3928, -8438),
        Point::new(-2304, 2679),
        Point::new(-219, -2281),
        Point::new(1238, -6814),
        Point::new(-7909, -4917),
        Point::new(-56, -8242),
        Point::new(-4487, 59),
        Point::new(550, 2368),
        Point::new(-1054, 5548),
        Point::new(-1389, 5862),
        Point::new(4967, -2804),
        Point::new(-6457, -3365),
        Point::new(4419, -693),
        Point::new(3128, -38),
        Point::new(-5167, -2436),
        Point::new(-489, 7647),
        Point::new(173, -6999),
        Point::new(4263, -5362),
        Point::new(5502, -3630),
        Point::new(-7797, -4590),
        Point::new(4108, -188),
        Point::new(4244, -2835),
        Point::new(-6275, -2569),
        Point::new(4007, 1559),
        Point::new(-3270, 3101),
        Point::new(-548, 4064),
        Point::new(-1187, 5091),
        Point::new(362, -3634),
        Point::new(-1477, 3818),
        Point::new(-3019, 3386),
        Point::new(-473, 5895),
        Point::new(1749, 1826),
        Point::new(-6605, -2446),
        Point::new(5373, -2166),
        Point::new(2790, 340),
        Point::new(2517, -6525),
        Point::new(-1015, -5440),
        Point::new(1327, 709),
        Point::new(-467, 5377),
        Point::new(-4537, 1089),
        Point::new(633, 2010),
        Point::new(-1894, 4739),
        Point::new(-1599, -8350),
        Point::new(5219, -506),
        Point::new(7635, -4185),
        Point::new(5650, -3879),
        Point::new(940, 5487),
        Point::new(-2698, -8664),
        Point::new(2539, 684),
        Point::new(-1015, -6439),
        Point::new(3106, 1770),
        Point::new(-1728, 4767),
        Point::new(-1760, 3744),
        Point::new(-951, -5308),
        Point::new(-2327, 3602),
        Point::new(2498, -1452),
        Point::new(-2442, -7518),
        Point::new(4092, 1057),
        Point::new(284, -4439),
        Point::new(1296, 4216),
        Point::new(-3731, 1188),
        Point::new(445, -7090),
        Point::new(-1406, -6666),
        Point::new(-112, 2242),
        Point::new(-4749, 482),
        Point::new(1697, 5304),
        Point::new(5334, -3939),
        Point::new(2304, 1648),
        Point::new(6157, -4079),
        Point::new(1272, -6979),
        Point::new(2332, -7483),
        Point::new(-1053, -4096),
        Point::new(3029, 2683),
        Point::new(-6817, -4790),
        Point::new(1059, 5473),
        Point::new(-3709, -159),
        Point::new(1299, -703),
        Point::new(-97, 7234),
        Point::new(3057, 1294),
        Point::new(604, -6253),
        Point::new(5935, -4216),
        Point::new(832, -3922),
        Point::new(-5475, -3087),
        Point::new(2658, -8807),
        Point::new(5356, -1433),
        Point::new(2208, 4168),
        Point::new(-6780, -2928),
        Point::new(-905, -6784),
        Point::new(-7492, -4591),
        Point::new(6253, -5307),
        Point::new(1923, 4332),
        Point::new(4107, -1996),
        Point::new(768, 7089),
        Point::new(3222, 2873),
        Point::new(5526, -2266),
        Point::new(-5473, -1446),
        Point::new(-3056, 736),
        Point::new(-674, 4328),
        Point::new(3396, 2071),
        Point::new(2328, 3609),
        Point::new(-2916, 3417),
        Point::new(-1185, 3618),
        Point::new(580, -5564),
        Point::new(-107, -7022),
        Point::new(5648, -5505),
        Point::new(-6665, -3556),
        Point::new(4887, 590),
        Point::new(1692, 589),
        Point::new(-2865, 1988),
        Point::new(-2530, -6293),
        Point::new(4114, -3672),
        Point::new(-5086, -1203),
        Point::new(4018, 1387),
        Point::new(4111, -2777),
        Point::new(6672, -3697),
        Point::new(-2203, -6839),
        Point::new(5424, -4827),
        Point::new(2673, 1694),
        Point::new(7666, -4521),
        Point::new(1761, 1360),
        Point::new(683, -3665),
        Point::new(2280, -8206),
        Point::new(3525, -513),
        Point::new(4181, 1048),
        Point::new(6783, -5487),
        Point::new(-1668, 5137),
        Point::new(601, 4712),
        Point::new(3038, -709),
        Point::new(-6332, -4252),
        Point::new(-3447, 1426),
        Point::new(737, 2621),
        Point::new(1063, 4678),
        Point::new(4027, -5465),
        Point::new(-1264, -4177),
        Point::new(-5220, -90),
        Point::new(1655, -7641),
        Point::new(-3765, -8589),
        Point::new(-811, 4604),
        Point::new(3915, -8354),
        Point::new(2917, 1360),
        Point::new(414, 5300),
        Point::new(-3253, 519),
        Point::new(-6176, -3308),
        Point::new(184, -8115),
        Point::new(-2107, -8378),
        Point::new(868, -7904),
        Point::new(3756, -2874),
        Point::new(207, -5438),
        Point::new(2359, -1644),
        Point::new(268, -5465),
        Point::new(5625, -1514),
        Point::new(5478, -4923),
        Point::new(235, -3315),
        Point::new(2875, 961),
        Point::new(1301, 6358),
        Point::new(-1131, -8141),
        Point::new(1210, -6224),
        Point::new(1008, -251),
        Point::new(3810, -4641),
        Point::new(-2323, -7478),
        Point::new(-967, -5708),
        Point::new(3889, -2205),
        Point::new(1344, 3088),
        Point::new(4393, -3872),
        Point::new(1067, 4243),
        Point::new(6954, -4505),
        Point::new(-4843, -860),
        Point::new(3198, -1413),
        Point::new(-2221, 3370),
        Point::new(2991, -2697),
        Point::new(3529, -2246),
        Point::new(6073, -3598),
        Point::new(-1268, -7039),
        Point::new(-5366, -220),
        Point::new(700, -7891),
        Point::new(70, -6630),
        Point::new(916, 2782),
        Point::new(507, -3632),
        Point::new(-2828, -6618),
        Point::new(2757, -1109),
        Point::new(2645, -1166),
        Point::new(-5294, -1578),
        Point::new(2890, 3200),
        Point::new(-137, 7255),
        Point::new(1580, 5245),
        Point::new(40, 2869),
        Point::new(-292, -4165),
        Point::new(-3200, 760),
        Point::new(450, -4289),
        Point::new(103, 1573),
        Point::new(-6830, -4490),
        Point::new(-1300, -7147),
        Point::new(2706, -2603),
        Point::new(6276, -1836),
        Point::new(1260, -333),
        Point::new(421, 7489),
        Point::new(5540, -3458),
        Point::new(399, -8009),
        Point::new(4728, -4865),
        Point::new(1859, -5905),
        Point::new(7128, -5198),
        Point::new(2755, 2726),
        Point::new(4198, 1487),
        Point::new(-6066, -1873),
        Point::new(-671, -7931),
        Point::new(-7257, -5012),
        Point::new(1578, 5716),
        Point::new(-1125, -7332),
        Point::new(-1937, -8418),
        Point::new(4066, -1807),
        Point::new(-230, 4114),
        Point::new(2377, -8061),
        Point::new(1690, 4749),
        Point::new(-681, -7976),
        Point::new(3111, -8384),
        Point::new(1953, 5631),
        Point::new(6286, -2051),
        Point::new(3298, -1542),
        Point::new(-1494, 5425),
        Point::new(1666, 1370),
        Point::new(-1371, 5524),
        Point::new(3442, -136),
        Point::new(2762, -6751),
        Point::new(2758, 1206),
        Point::new(184, 5115),
        Point::new(997, 5500),
        Point::new(4220, -3163),
        Point::new(4689, -1281),
        Point::new(256, 3464),
        Point::new(-1553, 3587),
        Point::new(3212, 1295),
        Point::new(551, 5120),
        Point::new(853, 5531),
        Point::new(3160, -1202),
        Point::new(3835, -4380),
        Point::new(-16, 3641),
        Point::new(3017, 3741),
        Point::new(-59, 8001),
        Point::new(5682, -1389),
        Point::new(-3438, 1435),
        Point::new(-1907, 4781),
        Point::new(4116, -8765),
        Point::new(1419, 5470),
        Point::new(-4728, -129),
        Point::new(4435, -5477),
        Point::new(403, -2892),
        Point::new(394, 5823),
        Point::new(6248, -2935),
        Point::new(-1431, -8717),
        Point::new(-1885, -8435),
        Point::new(-5107, -122),
        Point::new(4299, 259),
        Point::new(-1259, 3811),
        Point::new(796, -3834),
        Point::new(-6754, -3337),
        Point::new(-2348, -8288),
        Point::new(6618, -4856),
        Point::new(-6550, -4833),
        Point::new(3522, 1118),
        Point::new(1318, 4041),
        Point::new(1076, -6557),
        Point::new(3305, -1543),
        Point::new(-340, 6737),
        Point::new(-4799, 694),
        Point::new(356, 2005),
        Point::new(5206, -4015),
        Point::new(-2544, -8119),
        Point::new(285, 6483),
        Point::new(-3673, 1649),
        Point::new(775, 3137),
        Point::new(-2229, 4793),
        Point::new(-442, -7989),
        Point::new(-3000, 3381),
        Point::new(-1523, 4090),
        Point::new(-182, -4874),
        Point::new(-1646, 4627),
        Point::new(2806, 1816),
        Point::new(-1191, 5851),
        Point::new(4198, -2721),
        Point::new(-839, -8814),
        Point::new(7924, -5423),
        Point::new(4942, -3776),
        Point::new(1459, 1744),
        Point::new(-435, 5524),
        Point::new(-2768, 3007),
        Point::new(857, -4735),
        Point::new(829, 6644),
        Point::new(6867, -2876),
        Point::new(6591, -2629),
        Point::new(-5405, -489),
        Point::new(1329, 1192),
        Point::new(-185, -5114),
        Point::new(1217, 5717),
        Point::new(3320, 1715),
        Point::new(1852, -1200),
        Point::new(-235, -6545),
        Point::new(-4150, -757),
        Point::new(1942, 1570),
        Point::new(2739, 85),
        Point::new(1193, 5916),
        Point::new(1179, -5146),
        Point::new(2844, -6927),
        Point::new(5228, -3074),
        Point::new(-2302, -8245),
        Point::new(-2507, 3762),
        Point::new(595, -3832),
        Point::new(1113, -7435),
        Point::new(3917, 453),
        Point::new(2147, -7826),
        Point::new(7368, -5456),
        Point::new(-443, 4488),
        Point::new(-1049, -6000),
        Point::new(-1391, -6294),
        Point::new(-1530, 4157),
        Point::new(-3379, 278),
        Point::new(2414, -8506),
        Point::new(1137, 6730),
        Point::new(5156, -1878),
        Point::new(-401, 7568),
        Point::new(992, -5664),
        Point::new(986, 1968),
        Point::new(160, -4132),
        Point::new(6516, -4807),
        Point::new(1246, -8207),
        Point::new(-2094, 4525),
        Point::new(5076, -4515),
        Point::new(-2027, -7505),
        Point::new(-2726, 2230),
        Point::new(1258, 4149),
        Point::new(-345, 6717),
        Point::new(1147, 2316),
        Point::new(2029, -8575),
        Point::new(-6955, -4212),
        Point::new(1926, -1281),
        Point::new(6729, -2796),
        Point::new(802, 708),
        Point::new(2906, 469),
        Point::new(4487, 378),
        Point::new(601, -8689),
        Point::new(2025, 2483),
        Point::new(3548, -2416),
        Point::new(2765, -8436),
        Point::new(-198, 7847),
        Point::new(689, -7695),
        Point::new(3488, -2858),
        Point::new(-6466, -4848),
        Point::new(4929, -1969),
        Point::new(2714, 4250),
        Point::new(-34, -4796),
        Point::new(-5078, -2238),
        Point::new(1076, 692),
        Point::new(-6362, -4070),
        Point::new(-989, -3546),
        Point::new(1193, 2431),
        Point::new(694, -5130),
        Point::new(-678, 6319),
        Point::new(536, -6804),
        Point::new(-6362, -2117),
        Point::new(1369, 6410),
        Point::new(1358, 4407),
        Point::new(-4877, -1457),
        Point::new(6733, -3196),
        Point::new(-1232, -6973),
        Point::new(-3416, 1904),
        Point::new(6182, -3313),
        Point::new(7675, -4676),
        Point::new(-2119, -8409),
        Point::new(-5184, -2758),
        Point::new(507, 5135),
        Point::new(-7134, -4568),
        Point::new(-7173, -3657),
        Point::new(-4384, -384),
        Point::new(4366, -3546),
        Point::new(3739, -3870),
        Point::new(418, -7427),
        Point::new(-1715, 3967),
        Point::new(-695, -2884),
        Point::new(7357, -5011),
        Point::new(4095, -999),
        Point::new(-434, -8313),
        Point::new(4694, -4241),
        Point::new(3494, 2181),
        Point::new(-7202, -4197),
        Point::new(3709, -1270),
        Point::new(3121, -8773),
        Point::new(-4659, 237),
        Point::new(-322, -7873),
        Point::new(4614, -2904),
        Point::new(2446, 3729),
        Point::new(-430, -6549),
        Point::new(5092, -2558),
        Point::new(2547, 3378),
        Point::new(-4715, 830),
        Point::new(1965, 1031),
        Point::new(3957, -8780),
        Point::new(-724, 5529),
        Point::new(3531, 1979),
        Point::new(283, -6288),
        Point::new(391, -6110),
        Point::new(5299, -4908),
        Point::new(5581, -1644),
        Point::new(-2342, -7477),
        Point::new(2219, 1973),
        Point::new(6769, -4387),
        Point::new(-1266, 5774),
        Point::new(4537, -2569),
        Point::new(1008, 2891),
        Point::new(3305, -1652),
        Point::new(-6197, -3068),
        Point::new(7458, -5297),
        Point::new(930, 7219),
        Point::new(2305, -1105),
        Point::new(-2558, 2492),
        Point::new(655, -7749),
        Point::new(-2499, 4175),
        Point::new(-63, -5713),
        Point::new(-1745, -7437),
        Point::new(7990, -4914),
        Point::new(5784, -4854),
        Point::new(-54, -2379),
        Point::new(4634, -360),
        Point::new(-3188, 2683),
        Point::new(3723, -1695),
        Point::new(4385, -2823),
        Point::new(4810, 498),
        Point::new(-2525, -8292),
        Point::new(2397, -7973),
        Point::new(695, -3718),
        Point::new(-150, 8117),
        Point::new(6643, -4723),
        Point::new(1566, 1314),
        Point::new(-2585, -7303),
        Point::new(1806, 5856),
        Point::new(434, -5992),
        Point::new(1694, 4782),
        Point::new(7572, -4479),
        Point::new(7257, -5189),
        Point::new(5482, -4868),
        Point::new(-1902, -6477),
        Point::new(-190, -7750),
        Point::new(-826, -3352),
        Point::new(1982, -7856),
        Point::new(-3810, 247),
        Point::new(-5898, -1608),
        Point::new(4870, -2996),
        Point::new(-5821, -1358),
        Point::new(-3849, 1723),
        Point::new(570, -4273),
        Point::new(-1131, 6145),
        Point::new(-1308, -8090),
        Point::new(-161, -2490),
        Point::new(-7, -5636),
        Point::new(-583, -3316),
        Point::new(-3263, 1914),
        Point::new(-548, -4057),
        Point::new(6136, -3609),
        Point::new(3141, 255),
        Point::new(-768, -3152),
        Point::new(-3521, -8519),
        Point::new(-195, 7705),
        Point::new(-1152, 6074),
        Point::new(594, 1922),
        Point::new(-840, -8823),
        Point::new(-3128, -7170),
        Point::new(1316, -5408),
        Point::new(6291, -2702),
        Point::new(6090, -1854),
        Point::new(2898, 371),
        Point::new(-258, 5131),
        Point::new(652, -5980),
        Point::new(-1791, -7272),
        Point::new(4215, 1485),
        Point::new(1078, 2285),
        Point::new(395, -2481),
        Point::new(394, -8049),
        Point::new(1043, -7156),
        Point::new(-733, 3083),
        Point::new(-4493, 208),
        Point::new(3895, -4505),
        Point::new(-5619, -2764),
        Point::new(-1248, 4858),
        Point::new(-49, 6385),
        Point::new(-1511, -6952),
        Point::new(278, -4774),
        Point::new(343, -6673),
        Point::new(2418, 4753),
        Point::new(4146, -1808),
        Point::new(405, 7036),
        Point::new(1660, 2794),
        Point::new(-7618, -5018),
        Point::new(140, -2127),
        Point::new(1575, -5031),
        Point::new(-2028, -6027),
        Point::new(-1397, 6013),
        Point::new(1245, -7257),
        Point::new(3604, -2271),
        Point::new(137, 3303),
        Point::new(-5523, -771),
        Point::new(2998, 265),
        Point::new(-3066, -7726),
        Point::new(-1030, -7889),
        Point::new(6625, -4814),
        Point::new(5099, -2427),
        Point::new(-923, 6194),
        Point::new(655, 3496),
        Point::new(-3564, 2153),
        Point::new(4968, -3158),
        Point::new(-612, -4143),
        Point::new(1504, -8460),
        Point::new(-88, 1682),
        Point::new(-2328, -7267),
        Point::new(-2317, -7028),
        Point::new(-3513, 252),
        Point::new(3818, 937),
        Point::new(1854, -1651),
        Point::new(3728, 2122),
        Point::new(97, 4857),
        Point::new(-506, 4018),
        Point::new(3285, -3681),
        Point::new(6119, -4149),
        Point::new(2091, -6905),
        Point::new(178, -6860),
        Point::new(6954, -3122),
        Point::new(-1429, -5821),
        Point::new(-5569, -1698),
        Point::new(6561, -2797),
        Point::new(4408, -453),
        Point::new(1508, 4056),
        Point::new(1182, -194),
        Point::new(-2725, 2354),
        Point::new(2447, 780),
        Point::new(3423, 2719),
        Point::new(1914, -6293),
        Point::new(4675, -3340),
        Point::new(-1407, -5910),
        Point::new(-2856, -7132),
        Point::new(228, -3799),
        Point::new(-4050, -784),
        Point::new(4604, -2170),
        Point::new(2550, 1215),
        Point::new(1285, 1637),
        Point::new(478, 2913),
        Point::new(-148, 3552),
        Point::new(2364, 2611),
        Point::new(-1860, -4836),
        Point::new(-5831, -3333),
        Point::new(590, 1358),
        Point::new(1277, -5412),
        Point::new(-1259, -8350),
        Point::new(5698, -4208),
        Point::new(4550, -2794),
        Point::new(4032, -2645),
        Point::new(-1796, 5832),
        Point::new(7082, -4793),
        Point::new(2656, -6245),
        Point::new(-349, 3283),
        Point::new(-4108, 847),
        Point::new(1727, -44),
        Point::new(2483, 1349),
        Point::new(-658, 4690),
        Point::new(-1440, -7396),
        Point::new(378, 6127),
        Point::new(3089, 2702),
        Point::new(2794, -1655),
        Point::new(-4646, -1632),
        Point::new(208, 5862),
        Point::new(-4577, -552),
        Point::new(-2855, -8418),
        Point::new(-1745, 3421),
        Point::new(-6297, -2313),
        Point::new(1612, 1604),
        Point::new(-2038, -7170),
        Point::new(4478, 927),
        Point::new(-81, -8807),
        Point::new(4001, -1420),
        Point::new(-635, 4177),
        Point::new(5409, -4278),
        Point::new(7158, -3334),
        Point::new(94, -5845),
        Point::new(3085, -7565),
        Point::new(290, 4362),
        Point::new(2931, -7953),
        Point::new(-731, -5280),
        Point::new(4993, -2020),
        Point::new(7388, -3871),
        Point::new(7085, -5467),
        Point::new(5091, -3769),
        Point::new(4167, -4158),
        Point::new(-3775, 840),
        Point::new(3934, -3103),
        Point::new(-2927, -6978),
        Point::new(-3510, 982),
        Point::new(3170, 2768),
        Point::new(988, 57),
        Point::new(1357, 6143),
        Point::new(3261, -1886),
        Point::new(681, -7615),
        Point::new(1749, 615),
        Point::new(4612, 227),
        Point::new(-1858, 5297),
        Point::new(151, -5209),
        Point::new(2012, 4220),
        Point::new(3586, -3609),
        Point::new(-1479, -7522),
        Point::new(-7420, -4839),
        Point::new(2211, -6781),
        Point::new(5983, -3322),
        Point::new(-823, 3700),
        Point::new(-20, 9012),
        Point::new(1466, -8602),
        Point::new(-5465, -3150),
        Point::new(-1811, -8699),
        Point::new(-1541, 4321),
        Point::new(-2523, 4046),
        Point::new(2485, 4174),
        Point::new(1798, -5580),
        Point::new(1802, -69),
        Point::new(852, 4072),
        Point::new(-243, 7996),
        Point::new(5047, -2270),
        Point::new(-6999, -5059),
        Point::new(-1823, -7109),
        Point::new(-8, -4301),
        Point::new(1629, 952),
        Point::new(-595, -3641),
        Point::new(3224, 999),
        Point::new(-2813, -7474),
        Point::new(-2664, 3389),
        Point::new(2912, 3306),
        Point::new(0, -4655),
        Point::new(3520, -600),
        Point::new(114, -5005),
        Point::new(4719, -2418),
        Point::new(1579, 2421),
        Point::new(-2253, -6372),
        Point::new(1175, -4168),
        Point::new(-1779, 3221),
        Point::new(-427, -7109),
        Point::new(271, -8012),
        Point::new(5179, -1012),
        Point::new(-2998, -8676),
        Point::new(933, 6150),
        Point::new(5460, -1459),
        Point::new(-92, 1693),
        Point::new(767, 5758),
        Point::new(4167, -2912),
        Point::new(5389, -2387),
        Point::new(5411, -3682),
        Point::new(1435, -7680),
        Point::new(-5210, -2946),
        Point::new(-1256, 6420),
        Point::new(2154, 1712),
        Point::new(4007, -235),
        Point::new(-1175, 6567),
        Point::new(2294, -7580),
        Point::new(-1441, 4931),
        Point::new(3076, -3117),
        Point::new(-448, 7902),
        Point::new(4644, -5439),
        Point::new(514, 1735),
        Point::new(-1656, -6706),
        Point::new(802, -5943),
        Point::new(2435, -2096),
        Point::new(6257, -2209),
        Point::new(1896, -5336),
        Point::new(1162, -189),
        Point::new(-1549, -7782),
        Point::new(-170, 2362),
        Point::new(2413, 3339),
        Point::new(-534, -3992),
        Point::new(4668, -5192),
        Point::new(8127, -5159),
        Point::new(5064, -955),
        Point::new(-6318, -4869),
        Point::new(4114, -466),
        Point::new(-7683, -5468),
        Point::new(2251, 4852),
        Point::new(-2484, 2854),
        Point::new(-5829, -1496),
        Point::new(-1495, -7254),
        Point::new(183, -8291),
        Point::new(-1833, 2993),
        Point::new(2775, 1884),
        Point::new(495, -7415),
        Point::new(-1603, 5073),
        Point::new(-2094, 4613),
        Point::new(-3653, 470),
        Point::new(948, -8156),
        Point::new(3124, 2833),
        Point::new(1880, 3266),
        Point::new(-3214, 1273),
        Point::new(-1843, -6908),
        Point::new(250, -3434),
        Point::new(-6488, -3443),
        Point::new(235, 3453),
        Point::new(-675, 6410),
        Point::new(-5977, -3408),
        Point::new(-2676, 3922),
        Point::new(809, -8345),
        Point::new(2525, 2418),
        Point::new(2534, 2900),
        Point::new(-3219, 1390),
        Point::new(31, 3871),
        Point::new(4687, -4445),
        Point::new(3442, -2655),
        Point::new(617, 749),
        Point::new(-930, 5053),
        Point::new(6175, -2893),
        Point::new(2229, 2818),
        Point::new(3262, -2093),
        Point::new(-138, 4616),
        Point::new(3602, 1350),
        Point::new(168, 8757),
        Point::new(1604, -8340),
        Point::new(1550, -5764),
        Point::new(2074, -229),
        Point::new(1935, -1508),
        Point::new(270, 7102),
        Point::new(2672, 799),
        Point::new(2043, 2468),
        Point::new(5520, -1830),
        Point::new(-3552, -7998),
        Point::new(-1381, -5252),
        Point::new(4180, -1800),
        Point::new(3994, -987),
        Point::new(5652, -4095),
        Point::new(642, -6030),
        Point::new(-2285, 2690),
        Point::new(1139, 3243),
        Point::new(3609, -864),
        Point::new(-2674, 4010),
        Point::new(2908, -2978),
        Point::new(-6949, -4646),
        Point::new(-4747, 163),
        Point::new(-126, 4937),
        Point::new(-1200, 5714),
        Point::new(-1716, -5151),
        Point::new(17, -4312),
        Point::new(5987, -3829),
        Point::new(1218, -5744),
        Point::new(2253, -6439),
        Point::new(1943, -5993),
        Point::new(2920, -30),
        Point::new(-4486, -147),
        Point::new(-7261, -4955),
        Point::new(-303, 4817),
        Point::new(1024, 3452),
        Point::new(1568, 160),
        Point::new(554, -8160),
        Point::new(4365, -1083),
        Point::new(-2833, 2106),
        Point::new(5340, -2131),
        Point::new(3009, 2783),
        Point::new(-2003, -7597),
        Point::new(-7307, -3635),
        Point::new(-1694, -6636),
        Point::new(-1206, -3684),
        Point::new(4314, -1952),
        Point::new(-1846, 5397),
        Point::new(148, 4053),
        Point::new(1070, 3586),
        Point::new(1853, -7921),
        Point::new(-1255, -6615),
        Point::new(-4617, -34),
        Point::new(-2734, -8532),
        Point::new(-11, 2673),
        Point::new(-169, 1979),
        Point::new(-56, 7190),
        Point::new(-3721, -145),
        Point::new(-858, -4485),
        Point::new(-1883, -7720),
        Point::new(2739, -1160),
        Point::new(-347, -7247),
        Point::new(5456, -4465),
        Point::new(6552, -2406),
        Point::new(498, -5972),
        Point::new(4521, 358),
        Point::new(-2512, -5979),
        Point::new(-4604, 723),
        Point::new(5736, -3242),
        Point::new(-1018, 3566),
        Point::new(-2217, -8544),
        Point::new(-923, -6950),
        Point::new(5796, -2984),
        Point::new(-2588, 1574),
        Point::new(2820, -7072),
        Point::new(7257, -4885),
        Point::new(885, -4170),
        Point::new(4553, -3370),
        Point::new(1648, 4099),
        Point::new(-6688, -4784),
        Point::new(5499, -2935),
        Point::new(-4255, -502),
        Point::new(-55, 6125),
        Point::new(3434, -2843),
        Point::new(7274, -4143),
        Point::new(2904, 2601),
        Point::new(-1208, -7477),
        Point::new(1083, -6361),
        Point::new(65, 4960),
        Point::new(818, 207),
        Point::new(-2760, -6681),
        Point::new(2314, -858),
        Point::new(945, 3560),
        Point::new(-2923, 1641),
        Point::new(2751, -1450),
        Point::new(-4956, -2316),
        Point::new(-561, 6895),
        Point::new(-79, -3491),
        Point::new(137, 3846),
        Point::new(-7889, -4618),
        Point::new(886, 300),
        Point::new(675, -3404),
        Point::new(-725, 4336),
        Point::new(-7507, -4671),
        Point::new(1521, -1066),
        Point::new(1454, 5502),
        Point::new(-6907, -4999),
        Point::new(-2099, 2918),
        Point::new(1552, -5260),
        Point::new(454, -8632),
        Point::new(-3658, 1928),
        Point::new(1873, -5492),
        Point::new(1176, -4110),
        Point::new(3869, -2697),
        Point::new(-1403, -3992),
        Point::new(1165, 342),
        Point::new(2783, -51),
        Point::new(1474, -6942),
        Point::new(-832, 4919),
        Point::new(5029, -2566),
        Point::new(348, -2560),
        Point::new(755, -4328),
        Point::new(2901, 32),
        Point::new(1493, 3051),
        Point::new(498, 5617),
        Point::new(2884, -7048),
        Point::new(-3156, -8364),
        Point::new(3600, -397),
        Point::new(-371, 6318),
        Point::new(-334, -2563),
        Point::new(4371, -2424),
        Point::new(427, -7134),
        Point::new(-1932, 5095),
        Point::new(-1101, -7511),
        Point::new(-5966, -3556),
        Point::new(-3895, 1122),
        Point::new(91, -2919),
        Point::new(3486, -1360),
        Point::new(4019, 1020),
        Point::new(243, 2010),
        Point::new(1495, 3560),
        Point::new(-270, 4273),
        Point::new(-1194, -8320),
        Point::new(873, 1533),
        Point::new(-5350, -3011),
        Point::new(70, -6080),
        Point::new(-915, -6279),
        Point::new(3083, -354),
        Point::new(-1312, -5003),
        Point::new(252, 4025),
        Point::new(-1194, 5281),
        Point::new(-69, -4755),
        Point::new(-1102, -4186),
        Point::new(841, -8231),
        Point::new(3981, -2130),
        Point::new(7310, -5420),
        Point::new(3838, -2358),
        Point::new(-2073, 2857),
        Point::new(1725, 5367),
        Point::new(5146, -464),
        Point::new(1098, -49),
        Point::new(-202, -3301),
        Point::new(-3214, 716),
        Point::new(-1864, 3925),
        Point::new(884, -6382),
        Point::new(-2215, 3905),
        Point::new(-3703, 2471),
        Point::new(-6205, -4685),
        Point::new(-3154, 2335),
        Point::new(3071, 961),
        Point::new(1355, 5062),
        Point::new(-1084, 6317),
        Point::new(5877, -1234),
        Point::new(-586, 3755),
        Point::new(-805, 3468),
        Point::new(2980, -733),
        Point::new(800, 6975),
        Point::new(-826, 3900),
        Point::new(609, -2869),
        Point::new(-3871, -8416),
        Point::new(2072, 2599),
        Point::new(2642, -8157),
        Point::new(716, -7259),
        Point::new(-1456, -8063),
        Point::new(-5977, -2551),
        Point::new(-2562, -6238),
        Point::new(1936, -7320),
        Point::new(3127, 2556),
        Point::new(1971, 3939),
        Point::new(-5753, -1566),
        Point::new(4182, 1399),
        Point::new(-8198, -5510),
        Point::new(-2118, -7427),
        Point::new(1816, 4142),
        Point::new(803, -4515),
        Point::new(2847, -8800),
        Point::new(7979, -5355),
        Point::new(452, 3626),
        Point::new(-614, 3373),
        Point::new(-2118, -5424),
        Point::new(1021, -184),
        Point::new(-3789, -8451),
        Point::new(-1849, 4164),
        Point::new(-40, 5667),
        Point::new(3124, 3235),
        Point::new(-2000, 3327),
        Point::new(2074, 1392),
        Point::new(874, 2909),
        Point::new(1118, 3391),
        Point::new(-1270, -5739),
        Point::new(2179, 3102),
        Point::new(-108, 4820),
        Point::new(4324, -4841),
        Point::new(-6147, -2645),
        Point::new(-122, -5736),
        Point::new(2202, 2040),
        Point::new(5416, -4350),
        Point::new(-5948, -2893),
        Point::new(-2134, -8708),
        Point::new(2770, -6758),
        Point::new(1098, -8567),
        Point::new(-1655, 3951),
        Point::new(-130, 4519),
        Point::new(-1283, -4620),
        Point::new(1996, 2961),
        Point::new(-422, -8330),
        Point::new(-2976, 2451),
        Point::new(2992, -8108),
        Point::new(-937, 7069),
        Point::new(4537, -4419),
        Point::new(75, 6641),
        Point::new(149, -6147),
        Point::new(-3024, -8580),
        Point::new(-664, 4561),
        Point::new(-195, -7460),
        Point::new(-536, 7155),
        Point::new(1289, 2738),
        Point::new(466, 5656),
        Point::new(2946, 374),
        Point::new(4624, -5284),
        Point::new(6829, -3444),
        Point::new(105, -2108),
        Point::new(1421, -7933),
        Point::new(3859, -4101),
        Point::new(-6772, -3332),
        Point::new(2943, -3378),
        Point::new(-1634, 4134),
        Point::new(2383, -7885),
        Point::new(-1824, -7612),
        Point::new(-3465, -8017),
        Point::new(3364, -3384),
        Point::new(-1010, 6344),
        Point::new(3460, -3151),
        Point::new(346, 4960),
        Point::new(1337, -4040),
        Point::new(1170, 3994),
        Point::new(-3763, 1268),
        Point::new(1106, -5310),
        Point::new(1186, 2886),
        Point::new(1042, 6067),
        Point::new(5231, -448),
        Point::new(903, 4885),
        Point::new(217, 6525),
        Point::new(70, -6140),
        Point::new(59, -6178),
        Point::new(-492, -3754),
        Point::new(163, 6165),
        Point::new(-3857, 1221),
        Point::new(2685, 3092),
        Point::new(-1973, -6413),
        Point::new(1546, 2979),
        Point::new(-2348, -6936),
        Point::new(3767, -8828),
        Point::new(1908, -7172),
        Point::new(994, -6229),
        Point::new(2408, 3847),
        Point::new(3334, -528),
        Point::new(6063, -3842),
        Point::new(1690, -1287),
        Point::new(5934, -1345),
        Point::new(207, 3165),
        Point::new(-3112, 1470),
        Point::new(6360, -2387),
        Point::new(1350, -305),
        Point::new(-2331, -6155),
        Point::new(5678, -3664),
        Point::new(-2335, -7446),
        Point::new(1821, 4979),
        Point::new(-758, 4953),
        Point::new(-6852, -4480),
        Point::new(-4072, -378),
        Point::new(468, 7807),
        Point::new(4044, -617),
        Point::new(6351, -4989),
        Point::new(1205, 2743),
        Point::new(-675, -7606),
        Point::new(-2278, -7580),
        Point::new(3868, 186),
        Point::new(1664, 2102),
        Point::new(5603, -2396),
        Point::new(-1540, 5307),
        Point::new(-2470, 2322),
        Point::new(-2682, 2366),
        Point::new(1992, 3271),
        Point::new(-1149, 4420),
        Point::new(-3325, 540),
        Point::new(-111, 5399),
        Point::new(-655, -3224),
        Point::new(-3926, 223),
        Point::new(128, -7358),
        Point::new(-3837, 2124),
        Point::new(-195, 5551),
        Point::new(1058, -125),
        Point::new(1887, 1810),
        Point::new(1193, 2694),
        Point::new(-559, 6672),
        Point::new(-1662, 5231),
        Point::new(-3802, -144),
        Point::new(3289, -8775),
        Point::new(-1073, -6731),
        Point::new(-445, -3807),
        Point::new(-1676, -5575),
        Point::new(-1662, -8774),
        Point::new(1408, 5700),
        Point::new(-624, -6348),
        Point::new(233, 4244),
        Point::new(-816, 2992),
        Point::new(4723, -3603),
        Point::new(3946, -1554),
        Point::new(-814, -3900),
        Point::new(-648, 5242),
        Point::new(-3695, 1952),
        Point::new(1199, 3380),
        Point::new(1147, -7467),
        Point::new(-75, -4755),
        Point::new(-1258, 6228),
        Point::new(475, 6681),
        Point::new(959, 4698),
        Point::new(413, 6301),
        Point::new(-1333, -4296),
        Point::new(941, -3986),
        Point::new(-1246, -7941),
        Point::new(2766, -2078),
        Point::new(3946, -793),
        Point::new(-444, 4305),
        Point::new(-1519, -5300),
        Point::new(1295, -4254),
        Point::new(7519, -4524),
        Point::new(1640, 6177),
        Point::new(3263, -2744),
        Point::new(-5364, -2274),
        Point::new(-4474, 998),
        Point::new(-5537, -974),
        Point::new(4317, -1380),
        Point::new(1722, -537),
        Point::new(3003, -1139),
        Point::new(-4480, -1521),
        Point::new(1354, -8665),
        Point::new(-3285, 1561),
        Point::new(2176, 583),
        Point::new(-1797, -8190),
        Point::new(-4300, -325),
        Point::new(-6333, -1909),
        Point::new(-3486, 656),
        Point::new(2498, -1031),
        Point::new(-99, -8410),
        Point::new(443, 3621),
        Point::new(-192, 7733),
        Point::new(833, 4726),
        Point::new(-914, -5533),
        Point::new(3563, 704),
        Point::new(-7024, -5098),
        Point::new(4486, -1267),
        Point::new(-329, -2833),
        Point::new(2319, 3830),
        Point::new(-6051, -3372),
        Point::new(284, 3904),
        Point::new(931, -8176),
        Point::new(-719, 6644),
        Point::new(970, 6257),
        Point::new(-522, -7681),
        Point::new(887, 3371),
        Point::new(1689, -7564),
        Point::new(-2665, -7718),
        Point::new(-352, -4259),
        Point::new(1951, -685),
        Point::new(4637, -4011),
        Point::new(-7561, -4102),
        Point::new(316, 2089),
        Point::new(-1278, -7598),
        Point::new(2441, -7315),
        Point::new(-253, 8400),
        Point::new(828, -5728),
        Point::new(-337, -2546),
        Point::new(3553, -2517),
        Point::new(769, 3259),
        Point::new(206, -6397),
        Point::new(-1868, 5462),
        Point::new(3751, 1901),
        Point::new(-2403, 2546),
        Point::new(-815, -7141),
        Point::new(1360, 5904),
        Point::new(1463, 3567),
        Point::new(2139, 504),
        Point::new(3567, -3810),
        Point::new(5021, -1718),
        Point::new(3969, 1506),
        Point::new(-2156, 2566),
        Point::new(-1017, -4964),
        Point::new(2718, -1097),
        Point::new(-4031, 822),
        Point::new(-1447, -8100),
        Point::new(-1602, -4653),
        Point::new(-5255, -2529),
        Point::new(-1579, -7014),
        Point::new(-605, -5684),
        Point::new(-4481, 828),
        Point::new(968, 144),
        Point::new(-2346, 4982),
        Point::new(-3787, -8801),
        Point::new(936, 6755),
        Point::new(2063, -8176),
        Point::new(1212, 3028),
        Point::new(661, -5304),
        Point::new(-1157, 5762),
        Point::new(2039, -8274),
        Point::new(26, -6554),
        Point::new(-405, 3882),
        Point::new(2626, -2516),
        Point::new(6334, -4124),
        Point::new(-3049, -8346),
        Point::new(1270, -5189),
        Point::new(-7590, -5473),
        Point::new(-598, 5847),
        Point::new(3984, 818),
        Point::new(557, 585),
        Point::new(5806, -2157),
        Point::new(-3696, 1987),
        Point::new(-2387, -8134),
        Point::new(1931, -6664),
        Point::new(140, -6984),
        Point::new(1060, -7855),
        Point::new(217, -3748),
        Point::new(-2817, 4146),
        Point::new(-1387, 4474),
        Point::new(-2235, 3306),
        Point::new(2178, -8326),
        Point::new(1333, -4352),
        Point::new(-7073, -4742),
        Point::new(-2509, 3931),
        Point::new(3303, -2039),
        Point::new(-171, 2457),
        Point::new(1225, 4289),
        Point::new(1771, -7934),
        Point::new(-6606, -3041),
        Point::new(-3075, 1957),
        Point::new(578, 1720),
        Point::new(4293, -4030),
        Point::new(-3126, -8348),
        Point::new(-6239, -3005),
        Point::new(-2530, -6602),
        Point::new(-4000, 224),
        Point::new(-5121, -995),
        Point::new(2641, -1074),
        Point::new(-3043, -7186),
        Point::new(2965, 1200),
        Point::new(-2824, 3660),
        Point::new(4693, -2586),
        Point::new(1692, -1059),
        Point::new(6809, -2959),
        Point::new(-1748, 3091),
        Point::new(734, 4463),
        Point::new(2236, 3797),
        Point::new(5116, -4560),
        Point::new(-5047, -206),
        Point::new(4100, -2142),
        Point::new(-543, -8296),
        Point::new(1189, 5965),
        Point::new(2381, -1488),
        Point::new(2452, 1033),
        Point::new(1278, -7208),
        Point::new(3278, -8413),
        Point::new(-632, 6800),
        Point::new(1847, 3239),
        Point::new(4061, -1733),
        Point::new(3415, -3929),
        Point::new(5339, -2463),
        Point::new(-2939, -7750),
        Point::new(1481, -88),
        Point::new(-3644, 2136),
        Point::new(2038, 4955),
        Point::new(-348, -2780),
        Point::new(5355, -1814),
        Point::new(88, 2143),
        Point::new(1009, -5789),
        Point::new(5182, -4492),
        Point::new(-579, -5281),
        Point::new(-146, -6431),
        Point::new(712, -8361),
        Point::new(631, 4671),
        Point::new(-6538, -2392),
        Point::new(-89, -5560),
        Point::new(-589, 6810),
        Point::new(-1847, 3424),
        Point::new(216, 4873),
        Point::new(378, -7162),
        Point::new(-2323, -7114),
        Point::new(-3364, 2055),
        Point::new(3170, -8551),
        Point::new(-1073, 7022),
        Point::new(-2140, 3010),
        Point::new(1823, -6579),
        Point::new(1041, -8365),
        Point::new(3227, 3160),
        Point::new(2129, 1496),
        Point::new(366, 931),
        Point::new(-2723, -6526),
        Point::new(-4674, -1360),
        Point::new(375, 8288),
        Point::new(1518, -5844),
        Point::new(3614, 1860),
        Point::new(-696, 3598),
        Point::new(-1793, -8012),
        Point::new(-400, -3504),
        Point::new(-2560, -6561),
        Point::new(3624, 1778),
        Point::new(2783, 1413),
        Point::new(2842, 953),
        Point::new(-941, 4147),
        Point::new(1011, 6862),
        Point::new(-3850, -56),
        Point::new(88, 4186),
        Point::new(5348, -4507),
        Point::new(-7173, -5370),
        Point::new(-275, 2755),
        Point::new(3328, 2052),
        Point::new(614, -7169),
        Point::new(4089, -489),
        Point::new(4000, -4083),
        Point::new(-829, -8503),
        Point::new(-4332, -584),
        Point::new(-638, -8007),
        Point::new(674, 3236),
        Point::new(1385, -7905),
        Point::new(820, 6561),
        Point::new(1510, -7654),
        Point::new(788, 5187),
        Point::new(-1106, -4972),
        Point::new(989, 4387),
        Point::new(-1412, -8795),
        Point::new(6115, -5402),
        Point::new(-399, -4856),
        Point::new(174, -7478),
        Point::new(3658, 1584),
        Point::new(-2910, 2680),
        Point::new(2833, 3181),
        Point::new(3428, 200),
        Point::new(1210, 4097),
        Point::new(2419, -2293),
        Point::new(4192, -4812),
        Point::new(-1971, -7116),
        Point::new(-761, -8230),
        Point::new(-1152, 4942),
        Point::new(2564, 3236),
        Point::new(48, 2794),
        Point::new(1570, -5924),
        Point::new(3025, -8492),
        Point::new(4586, -3072),
        Point::new(-7016, -3734),
        Point::new(5332, -1689),
        Point::new(-1326, -4970),
        Point::new(947, -3640),
        Point::new(-1565, -5379),
        Point::new(3887, -769),
        Point::new(4443, -2833),
        Point::new(-6944, -5280),
        Point::new(-630, -7668),
        Point::new(679, -4891),
        Point::new(1219, 2077),
        Point::new(4617, -1974),
        Point::new(4933, -1260),
        Point::new(139, -6853),
        Point::new(-2386, 3228),
        Point::new(-3710, 498),
        Point::new(-2985, 2011),
        Point::new(1440, -8462),
        Point::new(3489, 1332),
        Point::new(4956, -2578),
        Point::new(802, -3909),
        Point::new(143, 2265),
        Point::new(-1340, -6987),
        Point::new(-292, -2222),
        Point::new(1975, -1237),
        Point::new(4370, -3675),
        Point::new(2110, -2083),
        Point::new(-5544, -2556),
        Point::new(3682, -1350),
        Point::new(-168, -7600),
        Point::new(1443, -4129),
        Point::new(-4101, 1719),
        Point::new(3512, 1752),
        Point::new(-5311, -1723),
        Point::new(1400, -479),
        Point::new(1292, 5939),
        Point::new(5858, -3160),
        Point::new(4916, -4272),
        Point::new(-468, -3403),
        Point::new(-829, -7898),
        Point::new(1421, -8004),
        Point::new(1232, -5762),
        Point::new(2863, 3061),
        Point::new(-861, 6207),
        Point::new(-362, 3889),
        Point::new(-823, -5337),
        Point::new(6420, -4060),
        Point::new(-1014, 5687),
        Point::new(-4799, 640),
        Point::new(2882, 2900),
        Point::new(2511, 2961),
        Point::new(3227, -1598),
        Point::new(1457, 3437),
        Point::new(1639, 4209),
        Point::new(1254, 5367),
        Point::new(611, 2974),
        Point::new(2461, -5989),
        Point::new(1725, 752),
        Point::new(6174, -3832),
        Point::new(1400, -4651),
        Point::new(256, -8804),
        Point::new(658, -4893),
        Point::new(-2744, 3006),
        Point::new(62, 1667),
        Point::new(-3026, 2297),
        Point::new(3234, -17),
        Point::new(7346, -5470),
        Point::new(1690, -5961),
        Point::new(7079, -3872),
        Point::new(-1561, 3824),
        Point::new(-3453, 290),
        Point::new(7003, -5218),
        Point::new(640, 4620),
        Point::new(-3946, -435),
        Point::new(4110, -5068),
        Point::new(2050, -1126),
        Point::new(1541, 2143),
        Point::new(-339, -2443),
        Point::new(-5462, -1891),
        Point::new(1996, 527),
        Point::new(-2395, -5887),
        Point::new(-3111, -7241),
        Point::new(2093, -8697),
        Point::new(7727, -5070),
        Point::new(-5909, -3147),
        Point::new(50, -5299),
        Point::new(-1010, 5170),
        Point::new(-606, -6509),
        Point::new(7010, -4803),
        Point::new(-5196, -1710),
        Point::new(1256, 1451),
        Point::new(-978, -3911),
        Point::new(1271, 4930),
        Point::new(2483, -7794),
        Point::new(-6330, -1976),
        Point::new(1921, -247),
        Point::new(3972, -8676),
        Point::new(577, 5489),
        Point::new(330, 6399),
        Point::new(6163, -4262),
        Point::new(-754, -5392),
        Point::new(-274, 4260),
        Point::new(-200, -6296),
        Point::new(-1675, -5182),
        Point::new(5898, -4973),
        Point::new(2880, -8209),
        Point::new(-4685, 477),
        Point::new(2495, 2887),
        Point::new(-3004, 2832),
        Point::new(-299, 6370),
        Point::new(-7384, -4083),
        Point::new(8320, -5448),
        Point::new(796, 5200),
        Point::new(6591, -3484),
        Point::new(105, -2868),
        Point::new(-2410, -7156),
        Point::new(2478, 3883),
        Point::new(599, -6789),
        Point::new(-3558, 2068),
        Point::new(1784, 3684),
        Point::new(-4261, 256),
        Point::new(1789, -6473),
        Point::new(3633, -3154),
        Point::new(1141, 4580),
        Point::new(-166, -6835),
        Point::new(-535, 7563),
        Point::new(2111, -1293),
        Point::new(4567, -1511),
        Point::new(6954, -3350),
        Point::new(1869, -5176),
        Point::new(926, 1454),
        Point::new(3592, 2187),
        Point::new(440, 3254),
        Point::new(3669, -8499),
        Point::new(-437, 2979),
        Point::new(1898, -5737),
        Point::new(-3123, 1108),
        Point::new(735, 5739),
        Point::new(-6480, -2574),
        Point::new(4279, -130),
        Point::new(-5492, -1003),
        Point::new(1846, -1359),
        Point::new(-3640, 1895),
        Point::new(2147, 1182),
        Point::new(3576, -2006),
        Point::new(-1307, 4007),
        Point::new(-1511, 4734),
        Point::new(1051, 437),
        Point::new(127, -7912),
        Point::new(1090, -3510),
        Point::new(167, 6149),
        Point::new(3228, -2325),
        Point::new(6713, -3303),
        Point::new(1893, 1144),
        Point::new(2293, 1570),
        Point::new(2779, -8332),
        Point::new(6550, -4033),
        Point::new(5932, -5235),
        Point::new(2353, -1919),
        Point::new(-1317, 4357),
        Point::new(6017, -3364),
        Point::new(1820, 2203),
        Point::new(-7992, -5062),
        Point::new(-5304, -746),
        Point::new(-593, 3871),
        Point::new(666, -4588),
        Point::new(2961, -7181),
        Point::new(-2675, -6650),
        Point::new(-1132, -4714),
        Point::new(5351, -1278),
        Point::new(122, 5572),
        Point::new(3140, -7865),
        Point::new(-4872, -2271),
        Point::new(-1686, 4631),
        Point::new(-1046, 5807),
        Point::new(3960, -4103),
        Point::new(4368, -1765),
        Point::new(-436, -4826),
        Point::new(1501, -8090),
        Point::new(-4824, 228),
        Point::new(-751, -5527),
        Point::new(1365, 293),
        Point::new(3958, -1850),
        Point::new(-142, -8073),
        Point::new(-1869, 4877),
        Point::new(-1042, 4380),
        Point::new(4083, -413),
        Point::new(-7869, -5209),
        Point::new(1026, 3421),
        Point::new(-2566, -7019),
        Point::new(-5360, -2355),
        Point::new(2097, 3764),
        Point::new(-3123, -7596),
        Point::new(-3756, 2427),
        Point::new(7245, -5083),
        Point::new(100, 5580),
        Point::new(2021, -1000),
        Point::new(1418, -8468),
        Point::new(3775, -4331),
        Point::new(-2749, 3230),
        Point::new(3138, -2178),
        Point::new(2488, -1818),
        Point::new(-2544, 1878),
        Point::new(6357, -2488),
        Point::new(3896, 172),
        Point::new(6790, -5274),
        Point::new(837, 5532),
        Point::new(607, 4121),
        Point::new(2411, 2096),
        Point::new(-467, 3219),
        Point::new(6413, -4868),
        Point::new(3562, 2390),
        Point::new(1561, -6272),
        Point::new(5958, -1900),
        Point::new(3354, -1713),
        Point::new(856, 1668),
        Point::new(1673, -6805),
        Point::new(-925, 6657),
        Point::new(-5125, -426),
        Point::new(7413, -5006),
        Point::new(6580, -5032),
        Point::new(-1472, -5188),
        Point::new(-203, 1997),
        Point::new(-3374, 2054),
        Point::new(-6997, -5292),
        Point::new(-5510, -1568),
        Point::new(4466, -2569),
        Point::new(40, -7950),
        Point::new(5879, -5200),
        Point::new(505, 6350),
        Point::new(-2547, 2901),
        Point::new(1459, -8359),
        Point::new(-3574, 2345),
        Point::new(1576, -4771),
        Point::new(1398, -620),
        Point::new(1071, 5162),
        Point::new(-167, -7083),
        Point::new(-5151, -1943),
        Point::new(-1744, 5810),
        Point::new(5593, -833),
        Point::new(103, -4932),
        Point::new(-2177, 2609),
        Point::new(-2432, -6514),
        Point::new(1057, 4260),
        Point::new(1470, 1297),
        Point::new(-135, 2983),
        Point::new(3979, -795),
        Point::new(1765, -8323),
        Point::new(1499, 302),
        Point::new(2941, -8372),
        Point::new(449, -4481),
        Point::new(954, 1502),
        Point::new(3714, -2834),
        Point::new(1359, -6141),
        Point::new(2492, 4303),
        Point::new(-738, -5395),
        Point::new(-5875, -3855),
        Point::new(1217, -6437),
        Point::new(1761, 1130),
        Point::new(1270, 4499),
        Point::new(826, -3823),
        Point::new(-1306, -5697),
        Point::new(742, 812),
        Point::new(-4295, -139),
        Point::new(3183, 1835),
        Point::new(2380, 3123),
        Point::new(-7262, -4134),
        Point::new(5704, -1098),
        Point::new(3973, 1484),
        Point::new(-335, -4963),
        Point::new(4562, 30),
        Point::new(1414, -487),
        Point::new(-2996, 2675),
        Point::new(1105, 3365),
        Point::new(2921, -6968),
        Point::new(4160, 1200),
        Point::new(5145, -1999),
        Point::new(-4466, -1436),
        Point::new(-4699, -266),
        Point::new(71, -4357),
        Point::new(-173, 6154),
        Point::new(-806, -7506),
        Point::new(2860, 2177),
        Point::new(-2436, 2283),
        Point::new(4105, -1936),
        Point::new(-972, -8148),
        Point::new(366, -3166),
        Point::new(1307, -305),
        Point::new(-6199, -4448),
        Point::new(-235, 8128),
        Point::new(1966, -6770),
        Point::new(-3260, -8390),
        Point::new(1948, 138),
        Point::new(732, -7741),
        Point::new(495, -6526),
        Point::new(1339, -8562),
        Point::new(3099, 962),
        Point::new(-1727, -6524),
        Point::new(5155, -2300),
        Point::new(5474, -2779),
        Point::new(-2258, -6542),
        Point::new(229, -6678),
        Point::new(143, 2426),
        Point::new(2644, -2289),
        Point::new(2520, -2527),
        Point::new(2529, 3152),
        Point::new(4169, 285),
        Point::new(2570, 2365),
        Point::new(-6595, -2630),
        Point::new(-1882, 4967),
        Point::new(6874, -3341),
        Point::new(1770, 4278),
        Point::new(-532, 6820),
        Point::new(140, 6552),
        Point::new(7090, -4061),
        Point::new(3587, 451),
        Point::new(7195, -4712),
        Point::new(-1067, -5633),
        Point::new(3481, 1900),
        Point::new(-624, 6441),
        Point::new(2071, -6297),
        Point::new(-4184, 813),
        Point::new(-665, 4155),
        Point::new(2207, 4084),
        Point::new(-7685, -4920),
        Point::new(-438, 3275),
        Point::new(-2995, 1542),
        Point::new(3056, -3679),
        Point::new(1764, -5992),
        Point::new(-4416, -1498),
        Point::new(-1564, -8796),
        Point::new(961, 6890),
        Point::new(-1366, -7742),
        Point::new(-141, -2971),
        Point::new(3272, 18),
        Point::new(3274, 2376),
        Point::new(1241, 3127),
        Point::new(-670, -3286),
        Point::new(-600, -5673),
        Point::new(-218, 4145),
        Point::new(1441, -1),
        Point::new(3816, -3453),
        Point::new(-3044, -8628),
        Point::new(-466, -4299),
        Point::new(-3052, -8421),
        Point::new(5751, -923),
        Point::new(4, -5689),
        Point::new(3122, -1593),
        Point::new(3422, -4287),
        Point::new(-2131, 5233),
        Point::new(808, -3345),
        Point::new(-277, -4823),
        Point::new(4823, -3746),
        Point::new(7227, -4955),
        Point::new(-3359, 2989),
        Point::new(-6013, -2361),
        Point::new(1279, -544),
        Point::new(4444, 1182),
        Point::new(-1558, 4619),
        Point::new(-3022, 2801),
        Point::new(-1527, -8579),
        Point::new(809, 6116),
        Point::new(-2584, -8424),
        Point::new(290, -6710),
        Point::new(4333, -3486),
        Point::new(-751, 7170),
        Point::new(-6039, -1480),
        Point::new(521, -7952),
        Point::new(5559, -4815),
        Point::new(347, 1561),
        Point::new(121, 7618),
        Point::new(4300, -4450),
        Point::new(2943, 102),
        Point::new(-3179, -7742),
        Point::new(4541, -4935),
        Point::new(-2078, -5454),
        Point::new(-1789, 4124),
        Point::new(5473, -609),
        Point::new(7927, -4674),
        Point::new(1060, -233),
        Point::new(-190, 4116),
        Point::new(-3254, -7768),
        Point::new(640, 4445),
        Point::new(-608, 7392),
        Point::new(863, -5287),
        Point::new(3974, -8546),
        Point::new(-5308, -2908),
        Point::new(3205, -3846),
        Point::new(4641, -895),
        Point::new(1880, -1002),
        Point::new(-4569, -1735),
        Point::new(-232, -5031),
        Point::new(2297, 4066),
        Point::new(4707, -4700),
        Point::new(1082, -8440),
        Point::new(-1152, -8468),
        Point::new(-1297, -8394),
        Point::new(888, 5645),
        Point::new(2795, -8277),
        Point::new(3481, 2276),
        Point::new(7502, -3934),
        Point::new(-5113, -1255),
        Point::new(-5446, -2488),
        Point::new(-6897, -4748),
        Point::new(-1277, 5785),
        Point::new(269, 5062),
        Point::new(3104, -8072),
        Point::new(2632, -284),
        Point::new(4410, 937),
        Point::new(-3478, 230),
        Point::new(4910, -4188),
        Point::new(-1861, 3497),
        Point::new(848, 4578),
        Point::new(4199, -5051),
        Point::new(1906, 5350),
        Point::new(-1254, -5857),
        Point::new(-2405, 3038),
        Point::new(-1772, -4715),
        Point::new(1135, 3443),
        Point::new(3116, -8140),
        Point::new(2854, 3683),
        Point::new(2519, -2286),
        Point::new(4425, -2714),
        Point::new(3213, 270),
        Point::new(3292, -3775),
        Point::new(-2342, -8100),
        Point::new(-2418, -6174),
        Point::new(4223, 1640),
        Point::new(2464, -8243),
        Point::new(-597, -5382),
        Point::new(2269, -2367),
        Point::new(2166, 4018),
        Point::new(-3593, -8503),
        Point::new(2039, 5361),
        Point::new(-2253, 2838),
        Point::new(-544, 3145),
        Point::new(-772, 6273),
        Point::new(-5807, -1919),
        Point::new(-7933, -5424),
        Point::new(2719, -8808),
        Point::new(2975, -925),
        Point::new(1031, 5161),
        Point::new(-2199, -6353),
        Point::new(-1919, 4730),
        Point::new(2992, 3155),
        Point::new(217, -5884),
        Point::new(5739, -3293),
        Point::new(466, 6635),
        Point::new(3474, 1347),
        Point::new(-1207, 6577),
        Point::new(106, 8719),
        Point::new(-7935, -4971),
        Point::new(-1932, 3116),
        Point::new(5162, -1234),
        Point::new(-3801, -8550),
        Point::new(-6042, -2654),
        Point::new(553, 7078),
        Point::new(-713, 7357),
        Point::new(-5532, -797),
        Point::new(1146, 1005),
        Point::new(803, 5956),
        Point::new(-94, 7710),
        Point::new(-5060, 27),
        Point::new(597, -5063),
        Point::new(-1200, 5674),
        Point::new(7367, -3694),
        Point::new(4199, -1203),
        Point::new(-706, -3329),
        Point::new(778, -4172),
        Point::new(-2881, 3106),
        Point::new(209, -3714),
        Point::new(-1340, -6899),
        Point::new(2138, 1842),
        Point::new(-5623, -1331),
        Point::new(2316, 2002),
        Point::new(863, 2977),
        Point::new(5144, -967),
        Point::new(1645, -4905),
        Point::new(5279, -5419),
        Point::new(-959, -5112),
        Point::new(1427, -7723),
        Point::new(478, -6823),
        Point::new(-994, -3553),
        Point::new(-2657, 2008),
        Point::new(-3760, 873),
        Point::new(-3147, -7763),
        Point::new(2643, 3656),
        Point::new(1909, -8211),
        Point::new(-6933, -3212),
        Point::new(-1994, -6820),
        Point::new(-5825, -3293),
        Point::new(134, 8467),
        Point::new(3161, 1932),
        Point::new(-2721, 2738),
        Point::new(-613, -7755),
        Point::new(5335, -2988),
        Point::new(7092, -4708),
        Point::new(-8195, -5322),
        Point::new(2213, -7355),
        Point::new(-5811, -1301),
        Point::new(2425, -2168),
        Point::new(8, 3049),
        Point::new(-579, -7736),
        Point::new(-687, 5763),
        Point::new(5188, -1220),
        Point::new(4203, -3835),
        Point::new(128, 6811),
        Point::new(-962, 5347),
        Point::new(-7660, -5050),
        Point::new(1785, -6115),
        Point::new(6044, -1833),
        Point::new(-6809, -3885),
        Point::new(-1278, -5462),
        Point::new(3608, -4044),
        Point::new(3337, 2126),
        Point::new(-783, -5928),
        Point::new(5243, -3517),
        Point::new(2378, -6311),
        Point::new(1401, -6219),
        Point::new(-5284, -2449),
        Point::new(295, -8292),
        Point::new(5488, -2004),
        Point::new(4737, -5070),
        Point::new(2869, 3048),
        Point::new(3775, 1146),
        Point::new(5127, -130),
        Point::new(4617, -1485),
        Point::new(1432, -596),
        Point::new(-855, -6100),
        Point::new(231, 8216),
        Point::new(6277, -5291),
        Point::new(-3446, 1380),
        Point::new(6664, -2591),
        Point::new(391, -2678),
        Point::new(4933, -1155),
        Point::new(3885, -5225),
        Point::new(7184, -5461),
        Point::new(-2208, -8643),
        Point::new(-6214, -1756),
        Point::new(2299, -624),
        Point::new(1398, 6318),
        Point::new(5406, -3764),
        Point::new(3458, -568),
        Point::new(-4175, -489),
        Point::new(7077, -5192),
        Point::new(2633, 3326),
        Point::new(-6915, -4350),
        Point::new(7277, -4385),
        Point::new(-1504, -5119),
        Point::new(4461, -2784),
        Point::new(-2778, 2717),
        Point::new(2736, -2040),
        Point::new(485, -5781),
        Point::new(1945, 4310),
        Point::new(-26, -8217),
        Point::new(4589, -2096),
        Point::new(4077, 4),
        Point::new(-749, -6537),
        Point::new(1711, -89),
        Point::new(4887, -2491),
        Point::new(944, 6346),
        Point::new(-6060, -2785),
        Point::new(1249, 2617),
        Point::new(-619, -4809),
        Point::new(-1899, 5155),
        Point::new(-956, -7809),
        Point::new(-4842, -1237),
        Point::new(3226, 2741),
        Point::new(-337, 3945),
        Point::new(2443, 355),
        Point::new(677, 1934),
        Point::new(-1485, -7369),
        Point::new(-1187, -8347),
        Point::new(-3271, 2628),
        Point::new(-3021, -8349),
        Point::new(3068, -2545),
        Point::new(-2354, 2489),
        Point::new(4111, -1205),
        Point::new(-6108, -3662),
        Point::new(1314, 5505),
        Point::new(1483, -4288),
        Point::new(2763, 1085),
        Point::new(-2261, 5031),
        Point::new(-605, -5967),
        Point::new(166, 5084),
        Point::new(4513, -5333),
        Point::new(-4162, -784),
        Point::new(2088, -6052),
        Point::new(2654, -6451),
        Point::new(1176, -7535),
        Point::new(2809, -746),
        Point::new(-5994, -1493),
        Point::new(3463, -3336),
        Point::new(-1607, 5391),
        Point::new(-1112, -7062),
        Point::new(2870, 1012),
        Point::new(-2406, 4848),
        Point::new(-3012, -7658),
        Point::new(2213, 4724),
        Point::new(-600, 3446),
        Point::new(-3133, -8746),
        Point::new(5156, -11),
        Point::new(5419, -3773),
        Point::new(4168, 975),
        Point::new(-778, -7782),
        Point::new(1133, -3654),
        Point::new(-1142, -4166),
        Point::new(4394, -4128),
        Point::new(457, 1108),
        Point::new(-5443, -3060),
        Point::new(1840, -6330),
        Point::new(-2698, 2479),
        Point::new(-2459, -6264),
        Point::new(2180, -8452),
        Point::new(2326, -7311),
        Point::new(-785, 5308),
        Point::new(-406, 7801),
        Point::new(3898, -8442),
        Point::new(-962, 3856),
        Point::new(-5857, -1478),
        Point::new(-6943, -4805),
        Point::new(2717, 1089),
        Point::new(7348, -4947),
        Point::new(5801, -3435),
        Point::new(-138, 4015),
        Point::new(-1738, 4502),
        Point::new(5747, -2614),
        Point::new(4429, -1881),
        Point::new(-543, -7145),
        Point::new(-4300, 1124),
        Point::new(2652, -409),
        Point::new(5260, -3052),
        Point::new(1219, -6798),
        Point::new(-618, 6037),
        Point::new(2294, -7103),
        Point::new(4658, -4460),
        Point::new(7472, -5228),
        Point::new(2253, -7011),
        Point::new(2289, -1868),
        Point::new(2911, -2810),
        Point::new(-2501, -6372),
        Point::new(-83, 7219),
        Point::new(5372, -3340),
        Point::new(-3430, 608),
        Point::new(468, 6954),
        Point::new(-6382, -3384),
        Point::new(-4174, 917),
        Point::new(1780, 3910),
        Point::new(-1030, 3427),
        Point::new(-1827, 5772),
        Point::new(1257, 1410),
        Point::new(5381, -639),
        Point::new(6333, -4745),
        Point::new(4178, 1695),
        Point::new(-3808, 609),
        Point::new(1800, -1526),
        Point::new(-7038, -5316),
        Point::new(-3044, -8283),
        Point::new(-404, -5965),
        Point::new(431, -7741),
        Point::new(8294, -5314),
        Point::new(3262, 169),
        Point::new(4183, -1578),
        Point::new(-6661, -4873),
        Point::new(5166, -5241),
        Point::new(2199, -1140),
        Point::new(-515, 6624),
        Point::new(1152, -8529),
        Point::new(-1382, -5007),
        Point::new(1935, -5345),
        Point::new(1651, 1960),
        Point::new(4673, -3652),
        Point::new(6589, -2554),
        Point::new(-2534, -6747),
        Point::new(-402, 7513),
        Point::new(3860, -4371),
        Point::new(2265, -6857),
        Point::new(-611, 5869),
        Point::new(4039, -2907),
        Point::new(2879, 3832),
        Point::new(2691, 609),
        Point::new(-6153, -3601),
        Point::new(-8219, -5290),
        Point::new(-1857, 5119),
        Point::new(4375, -217),
        Point::new(126, 1429),
        Point::new(1350, -7371),
        Point::new(1629, 5654),
        Point::new(-364, 3909),
        Point::new(1165, -6658),
        Point::new(1614, 1396),
        Point::new(5721, -4514),
        Point::new(1842, 2872),
        Point::new(832, 4953),
        Point::new(41, -4790),
        Point::new(-7041, -3582),
        Point::new(2667, -2806),
        Point::new(2153, -1970),
        Point::new(-1470, 5306),
        Point::new(-3521, 391),
        Point::new(-271, 5424),
        Point::new(-979, -8518),
        Point::new(1041, -4701),
        Point::new(7008, -5087),
        Point::new(-5577, -1439),
        Point::new(-6352, -4590),
        Point::new(4843, -340),
        Point::new(-3323, 2888),
        Point::new(-6252, -1980),
        Point::new(2349, 4706),
        Point::new(4249, -2689),
        Point::new(-6722, -4358),
        Point::new(4890, -375),
        Point::new(-1823, 3669),
        Point::new(467, 1444),
        Point::new(3863, -5030),
        Point::new(2475, -8182),
        Point::new(711, -3347),
        Point::new(1808, 4397),
        Point::new(3716, -3005),
        Point::new(-607, -7627),
        Point::new(3122, -248),
        Point::new(-1141, 3874),
        Point::new(3075, 488),
        Point::new(-5246, -2492),
        Point::new(-447, -7775),
        Point::new(638, 4407),
        Point::new(1207, -8542),
        Point::new(-5749, -3696),
        Point::new(3511, 959),
        Point::new(1611, -6322),
        Point::new(-82, 7568),
        Point::new(-120, 8127),
        Point::new(-3653, 271),
        Point::new(-6572, -5315),
        Point::new(6939, -3769),
        Point::new(-326, 3852),
        Point::new(1336, -4920),
        Point::new(3346, 858),
        Point::new(-2909, 4017),
        Point::new(1518, -6737),
        Point::new(559, 4953),
        Point::new(3007, -8738),
        Point::new(2046, 67),
        Point::new(-671, 5975),
        Point::new(-1240, 4110),
        Point::new(468, -3707),
        Point::new(-2223, -8693),
        Point::new(-6641, -4516),
        Point::new(-4989, -1960),
        Point::new(-657, 7779),
        Point::new(3448, -1357),
        Point::new(3970, 1031),
        Point::new(-603, -6732),
        Point::new(-5415, -2777),
        Point::new(3885, 71),
        Point::new(7078, -4887),
        Point::new(3692, -7945),
        Point::new(3579, -3145),
        Point::new(1056, -5411),
        Point::new(2385, 3842),
        Point::new(1390, -6217),
        Point::new(-4517, 925),
        Point::new(-769, 4442),
        Point::new(-537, -8332),
        Point::new(-729, -5144),
        Point::new(-613, 3805),
        Point::new(2057, 2729),
        Point::new(-4054, 64),
        Point::new(-118, -4640),
        Point::new(1445, 4986),
        Point::new(1925, -6146),
        Point::new(-3285, 681),
        Point::new(-1394, -7349),
        Point::new(-1014, 3627),
        Point::new(1990, -7866),
        Point::new(3914, -2015),
        Point::new(3461, 2414),
        Point::new(-6148, -3431),
        Point::new(2844, -2528),
        Point::new(-186, -7401),
        Point::new(13, 3427),
        Point::new(-4158, 386),
        Point::new(-1301, -8445),
        Point::new(-1621, 6131),
        Point::new(-2525, 4026),
        Point::new(2316, -7424),
        Point::new(2364, 803),
        Point::new(2210, 5098),
        Point::new(6010, -3982),
        Point::new(105, -4492),
        Point::new(3979, 1122),
        Point::new(-3765, 495),
        Point::new(342, -7317),
        Point::new(2445, 549),
        Point::new(1446, 5532),
        Point::new(-6623, -3747),
        Point::new(3635, -438),
        Point::new(3842, -8614),
        Point::new(5084, -3177),
        Point::new(4063, 1398),
        Point::new(-566, 4767),
        Point::new(-1367, -7003),
        Point::new(5025, -4203),
        Point::new(-8120, -5283),
        Point::new(4525, -920),
        Point::new(300, -7505),
        Point::new(-3909, -238),
        Point::new(-2699, 3551),
        Point::new(-3248, 1126),
        Point::new(2370, 921),
        Point::new(3521, 350),
        Point::new(-1478, 6371),
        Point::new(1259, -4849),
        Point::new(56, -8155),
        Point::new(-1367, 5149),
        Point::new(6008, -5119),
        Point::new(1862, 3965),
        Point::new(2079, 3667),
        Point::new(515, 625),
        Point::new(4875, -2351),
        Point::new(-1055, -7483),
        Point::new(82, -5069),
        Point::new(-4551, -1828),
        Point::new(1404, -6856),
        Point::new(2880, -717),
        Point::new(6353, -3759),
        Point::new(4540, -496),
        Point::new(-395, -4150),
        Point::new(-6176, -2847),
        Point::new(-1757, -6340),
        Point::new(2352, 2612),
        Point::new(-285, 8286),
        Point::new(1831, -5733),
        Point::new(158, -4208),
        Point::new(2365, 3355),
        Point::new(-5031, -430),
        Point::new(3608, -1129),
        Point::new(-3488, -8226),
        Point::new(1668, -7737),
        Point::new(500, 5873),
        Point::new(-5990, -3213),
        Point::new(-1036, -8571),
        Point::new(7223, -4226),
        Point::new(-2524, 2309),
        Point::new(-6163, -4535),
        Point::new(3960, -328),
        Point::new(-2268, 3794),
        Point::new(-1381, -7771),
        Point::new(1945, 213),
        Point::new(-4563, -84),
        Point::new(839, 7159),
        Point::new(3000, 3226),
        Point::new(-398, -6479),
        Point::new(-162, -3557),
        Point::new(5166, -2354),
        Point::new(-1044, 6272),
        Point::new(1053, 578),
        Point::new(-1904, -5110),
        Point::new(1856, 1718),
        Point::new(3504, -826),
        Point::new(3296, 3033),
        Point::new(-271, -3531),
        Point::new(691, 1531),
        Point::new(795, 5449),
        Point::new(3375, -8509),
        Point::new(-224, 7900),
        Point::new(6032, -1729),
        Point::new(751, 4393),
        Point::new(488, -7503),
        Point::new(3956, -1657),
        Point::new(-135, 7324),
        Point::new(-1429, 5587),
        Point::new(-266, -5785),
        Point::new(-822, 5363),
        Point::new(2857, -7042),
        Point::new(1921, -1633),
        Point::new(1889, 3547),
        Point::new(2066, 985),
        Point::new(1837, -7347),
        Point::new(3775, -461),
        Point::new(4722, -1842),
        Point::new(287, 5690),
        Point::new(-7033, -3417),
        Point::new(5467, -1330),
        Point::new(-1581, 5339),
        Point::new(-7598, -4438),
        Point::new(3266, -271),
        Point::new(1848, -1066),
        Point::new(6536, -2468),
        Point::new(-133, 5948),
        Point::new(3039, -8046),
        Point::new(2999, -2427),
        Point::new(82, -2889),
        Point::new(2118, 804),
        Point::new(-76, 6082),
        Point::new(873, -5944),
        Point::new(383, 3222),
        Point::new(-2381, -5748),
        Point::new(7244, -5083),
        Point::new(670, -8794),
        Point::new(-6673, -4752),
        Point::new(-5891, -2833),
        Point::new(-1556, -8338),
        Point::new(-4, 4545),
        Point::new(1774, 4081),
        Point::new(-1043, -4876),
        Point::new(-567, 2793),
        Point::new(-3922, 179),
        Point::new(-371, -5041),
        Point::new(1227, -3778),
        Point::new(1476, 2532),
        Point::new(2034, -1450),
        Point::new(3628, -4433),
        Point::new(1522, -168),
        Point::new(-5312, -2585),
        Point::new(371, -7896),
        Point::new(-432, -4434),
        Point::new(-181, 1854),
        Point::new(6182, -4599),
        Point::new(3406, -3665),
        Point::new(484, 1508),
        Point::new(4727, -3197),
        Point::new(-3351, -8071),
        Point::new(5162, -1178),
        Point::new(-1403, 5660),
        Point::new(-1954, 2760),
        Point::new(2765, -993),
        Point::new(4935, -5256),
        Point::new(2036, -1928),
        Point::new(720, 2533),
        Point::new(2584, -2252),
        Point::new(2072, -173),
        Point::new(-2697, 4104),
        Point::new(5457, -2679),
        Point::new(6858, -4845),
        Point::new(3920, -8703),
        Point::new(-6848, -3120),
        Point::new(-4302, -758),
        Point::new(-407, 5462),
        Point::new(3199, -7174),
        Point::new(2360, -7695),
        Point::new(7989, -5055),
        Point::new(-2767, -8450),
        Point::new(4035, -8580),
        Point::new(-2476, -6109),
        Point::new(4968, -4371),
        Point::new(-285, 2592),
        Point::new(5550, -4957),
        Point::new(1858, 1761),
        Point::new(-3773, 634),
        Point::new(-763, -6068),
        Point::new(-1445, -4204),
        Point::new(4063, -288),
        Point::new(-1066, -4179),
        Point::new(2506, -8447),
        Point::new(-4420, 492),
        Point::new(6372, -4960),
        Point::new(490, -3952),
        Point::new(-2915, 2222),
        Point::new(-98, 8279),
        Point::new(1550, 4778),
        Point::new(1419, 5464),
        Point::new(-6619, -5155),
        Point::new(-1118, -4369),
        Point::new(-5235, -1516),
        Point::new(1095, -4674),
        Point::new(8011, -4871),
        Point::new(1916, -7534),
        Point::new(6665, -4960),
        Point::new(4033, -2933),
        Point::new(-154, 8431),
        Point::new(6512, -4172),
        Point::new(667, 1766),
        Point::new(5640, -2647),
        Point::new(-2280, 2449),
        Point::new(4750, -5338),
        Point::new(1013, 2279),
        Point::new(678, 4912),
        Point::new(-5227, -1629),
        Point::new(5397, -4300),
        Point::new(1114, 2982),
        Point::new(1330, 3566),
        Point::new(2170, -1811),
        Point::new(2703, 466),
        Point::new(-5866, -1784),
        Point::new(4377, -3895),
        Point::new(-706, -5346),
        Point::new(-600, 7581),
        Point::new(-2711, -7719),
        Point::new(-1419, -7122),
        Point::new(5726, -1230),
        Point::new(-5941, -1854),
        Point::new(-2558, 2619),
        Point::new(-5266, -2736),
        Point::new(6326, -3385),
        Point::new(1495, 3481),
        Point::new(3705, -1159),
        Point::new(446, 7460),
        Point::new(1624, 377),
        Point::new(3189, -7713),
        Point::new(2625, -2017),
        Point::new(196, 8299),
        Point::new(5043, -2448),
        Point::new(-2476, -8245),
        Point::new(5461, -3514),
        Point::new(581, 3032),
        Point::new(2657, -8607),
        Point::new(228, -6782),
        Point::new(-890, -7681),
        Point::new(672, 387),
        Point::new(7129, -5391),
        Point::new(-1853, -7900),
        Point::new(2032, 56),
        Point::new(3654, 2192),
        Point::new(600, 5793),
        Point::new(1827, 3631),
        Point::new(-587, -4320),
        Point::new(-3778, 869),
        Point::new(939, -5798),
        Point::new(4125, -2159),
        Point::new(3575, -3718),
        Point::new(1863, 3902),
        Point::new(2486, -8223),
        Point::new(526, 2166),
        Point::new(533, -8227),
        Point::new(1511, 862),
        Point::new(2293, 2126),
        Point::new(1867, 2698),
        Point::new(360, 1129),
        Point::new(2914, -7995),
        Point::new(-549, 3422),
        Point::new(-3468, -8772),
        Point::new(4190, -2939),
        Point::new(1848, -6399),
        Point::new(2055, 4347),
        Point::new(4933, -2895),
        Point::new(2930, 3901),
        Point::new(1129, 3026),
        Point::new(-864, -5513),
        Point::new(5255, -3275),
        Point::new(6212, -1999),
        Point::new(-657, 4037),
        Point::new(2882, -630),
        Point::new(1524, -8542),
        Point::new(718, 2428),
        Point::new(1790, -5487),
        Point::new(3037, -454),
        Point::new(1344, 2031),
        Point::new(1281, 2719),
        Point::new(-1101, 4689),
        Point::new(96, -8429),
        Point::new(181, 3190),
        Point::new(-2217, 4574),
        Point::new(1548, -259),
        Point::new(179, 6826),
        Point::new(-6275, -4136),
        Point::new(773, 6125),
        Point::new(4176, -1140),
        Point::new(3363, -1200),
        Point::new(7066, -3977),
        Point::new(1253, -114),
        Point::new(997, 6037),
        Point::new(168, 1244),
        Point::new(-115, 2616),
        Point::new(-6418, -3796),
        Point::new(4590, -1554),
        Point::new(4498, -1343),
        Point::new(-3325, -8138),
        Point::new(-3165, 2038),
        Point::new(-7645, -4994),
        Point::new(864, -3443),
        Point::new(854, -7242),
        Point::new(-3004, -8017),
        Point::new(-8131, -5432),
        Point::new(3058, -3539),
        Point::new(-495, -3428),
        Point::new(-4564, -736),
        Point::new(6879, -4959),
        Point::new(-2478, -7367),
        Point::new(-3222, 718),
        Point::new(149, 1451),
        Point::new(309, -4142),
        Point::new(489, -4687),
        Point::new(233, 4694),
        Point::new(578, -6645),
        Point::new(709, -7628),
        Point::new(-882, -6663),
        Point::new(1465, 4814),
        Point::new(-5428, -645),
        Point::new(783, -3482),
        Point::new(7261, -5346),
        Point::new(-958, -8133),
        Point::new(-833, 4375),
        Point::new(7221, -4781),
        Point::new(1987, 79),
        Point::new(-417, -5639),
        Point::new(-534, 6530),
        Point::new(-1531, 3965),
        Point::new(322, 4130),
        Point::new(2072, -6755),
        Point::new(1401, -97),
        Point::new(-1183, 6666),
        Point::new(-6605, -5197),
        Point::new(-818, 6758),
        Point::new(4248, 1222),
        Point::new(1693, 1250),
        Point::new(4589, 975),
        Point::new(3715, -3467),
        Point::new(-4738, -577),
        Point::new(2781, -1401),
        Point::new(-1441, 3884),
        Point::new(201, 8685),
        Point::new(2186, -2152),
        Point::new(6081, -4209),
        Point::new(-7245, -4547),
        Point::new(648, -7815),
        Point::new(-7908, -5307),
        Point::new(771, 6416),
        Point::new(4159, -4470),
        Point::new(-3935, 929),
        Point::new(-936, -6981),
        Point::new(5213, -3125),
        Point::new(-6404, -4755),
        Point::new(2301, 2057),
        Point::new(-3819, 2054),
        Point::new(2153, -5606),
        Point::new(3839, -8714),
        Point::new(-2682, 4061),
        Point::new(-1469, 5109),
        Point::new(-6510, -4733),
        Point::new(292, -3329),
        Point::new(3935, -2918),
        Point::new(1566, -274),
        Point::new(4562, -3847),
        Point::new(-7480, -5348),
        Point::new(-334, 3525),
        Point::new(-3866, -8394),
        Point::new(-739, -4458),
        Point::new(4247, -3301),
        Point::new(791, -7275),
        Point::new(362, 897),
        Point::new(2491, -8303),
        Point::new(-1597, -5256),
        Point::new(-1772, 4028),
        Point::new(1954, -6681),
        Point::new(-6034, -1487),
        Point::new(-1975, -6638),
        Point::new(-3526, 2753),
        Point::new(-234, -8600),
        Point::new(3934, -4065),
        Point::new(3866, -2884),
        Point::new(1369, -4278),
        Point::new(1680, 3613),
        Point::new(2039, -7616),
        Point::new(342, 2957),
        Point::new(-3968, -8433),
        Point::new(-1978, 2716),
        Point::new(-563, 3661),
        Point::new(1224, 2460),
        Point::new(1947, -8786),
        Point::new(4205, 843),
        Point::new(742, 5847),
        Point::new(-6898, -4806),
        Point::new(3308, -8331),
        Point::new(5417, -4493),
        Point::new(3365, -2918),
        Point::new(2696, -8371),
        Point::new(2109, -5884),
        Point::new(3533, -8290),
        Point::new(428, 6092),
        Point::new(132, 5824),
        Point::new(365, -8076),
        Point::new(-4317, 1479),
        Point::new(-3477, 2551),
        Point::new(5503, -2361),
        Point::new(-2535, 4460),
        Point::new(3985, 1015),
        Point::new(5260, -4675),
        Point::new(1204, 6607),
        Point::new(2208, -8242),
        Point::new(-3110, -8772),
        Point::new(2073, -891),
        Point::new(-5402, -2253),
        Point::new(1951, 4659),
        Point::new(2080, -5836),
        Point::new(1291, -7576),
        Point::new(4553, -2220),
        Point::new(691, -4749),
        Point::new(-2180, -7909),
        Point::new(-1361, -5014),
        Point::new(1067, 2813),
        Point::new(8, -2764),
        Point::new(6070, -2419),
        Point::new(743, 2406),
        Point::new(1297, 4603),
        Point::new(-720, 7675),
        Point::new(-3154, -7609),
        Point::new(-1559, -4415),
        Point::new(2463, 2222),
        Point::new(3733, -8744),
        Point::new(5335, -3454),
        Point::new(3819, -8209),
        Point::new(2979, -1604),
        Point::new(-579, 5214),
        Point::new(1089, 1516),
        Point::new(-948, -8029),
        Point::new(6073, -2708),
        Point::new(-1099, 6152),
        Point::new(366, 2639),
        Point::new(5226, -2641),
        Point::new(1960, 4012),
        Point::new(-1425, -8701),
        Point::new(3175, -8501),
        Point::new(-6462, -4461),
        Point::new(1779, -37),
        Point::new(-102, 7730),
        Point::new(-2291, 4992),
        Point::new(1464, 4851),
        Point::new(1200, 6190),
        Point::new(-891, 4165),
        Point::new(5868, -4993),
        Point::new(-1090, 5802),
        Point::new(1755, 209),
        Point::new(-4437, -105),
        Point::new(-787, -7465),
        Point::new(-2162, -6722),
        Point::new(4233, 805),
        Point::new(1225, -7594),
        Point::new(6096, -3184),
        Point::new(2436, -8141),
        Point::new(3937, -5212),
        Point::new(-708, -8424),
        Point::new(1289, -7744),
        Point::new(3259, 3241),
        Point::new(3747, -170),
        Point::new(300, 3223),
        Point::new(-947, 3134),
        Point::new(-879, 7309),
        Point::new(-356, 4508),
        Point::new(3385, -3058),
        Point::new(1782, 2825),
        Point::new(823, 4080),
        Point::new(-5778, -3331),
        Point::new(3480, 2113),
        Point::new(-2356, -7335),
        Point::new(4593, -2238),
        Point::new(733, -7641),
        Point::new(-268, -3050),
        Point::new(1323, -3956),
        Point::new(-6513, -2467),
        Point::new(5320, -2090),
        Point::new(2856, 1350),
        Point::new(1394, -6502),
        Point::new(-193, 4504),
        Point::new(455, -6273),
        Point::new(-6650, -5391),
        Point::new(7692, -4565),
        Point::new(4767, -3436),
        Point::new(64, 7400),
        Point::new(31, -6534),
        Point::new(1314, -7276),
        Point::new(988, -8418),
        Point::new(5896, -4849),
        Point::new(5250, -467),
        Point::new(-5927, -4169),
        Point::new(-7380, -5021),
        Point::new(1427, 1145),
        Point::new(-158, 7520),
        Point::new(-2809, -7956),
        Point::new(-1017, 6564),
        Point::new(2189, 3315),
        Point::new(5743, -1132),
        Point::new(-4959, -1503),
        Point::new(6079, -4556),
        Point::new(3246, 745),
        Point::new(5913, -5033),
        Point::new(2346, -846),
        Point::new(6884, -5136),
        Point::new(1633, 427),
        Point::new(-3400, 1021),
        Point::new(-2720, 4179),
        Point::new(534, 5552),
        Point::new(4620, -2003),
        Point::new(-1135, 3589),
        Point::new(-1139, 6359),
        Point::new(-5679, -1958),
        Point::new(807, -3263),
        Point::new(4443, -4563),
        Point::new(3263, 2291),
        Point::new(906, -8828),
        Point::new(2396, -7960),
        Point::new(-914, -8379),
        Point::new(-2351, -8621),
        Point::new(-533, -4199),
        Point::new(5820, -1700),
        Point::new(47, -3127),
        Point::new(625, -7763),
        Point::new(-304, -7133),
        Point::new(-6588, -4149),
        Point::new(1063, 4947),
        Point::new(6857, -5414),
        Point::new(1064, -4972),
        Point::new(4969, -170),
        Point::new(1480, 5107),
        Point::new(3907, -3582),
        Point::new(118, -6174),
        Point::new(-1521, -6361),
        Point::new(-2724, -6406),
        Point::new(1829, -6747),
        Point::new(4142, -2819),
        Point::new(850, -3671),
        Point::new(577, 1458),
        Point::new(5559, -5394),
        Point::new(307, 5501),
        Point::new(3268, -3386),
        Point::new(-2977, -7901),
        Point::new(2353, 1534),
        Point::new(-226, 7400),
        Point::new(-3615, 1978),
        Point::new(-918, 6691),
        Point::new(-5814, -1906),
        Point::new(4613, -1947),
        Point::new(2651, -2619),
        Point::new(-610, -5088),
        Point::new(55, 7286),
        Point::new(4448, -2337),
        Point::new(1594, -8052),
        Point::new(-1276, -5520),
        Point::new(-4167, -472),
        Point::new(6884, -4558),
        Point::new(4942, -3303),
        Point::new(-3179, 2471),
        Point::new(-3249, 465),
        Point::new(-3700, 794),
        Point::new(-97, 7637),
        Point::new(-1989, 5567),
        Point::new(7813, -5023),
        Point::new(6279, -2779),
        Point::new(1416, 5172),
        Point::new(-2, 1889),
        Point::new(3779, 2218),
        Point::new(997, 4220),
        Point::new(5905, -3851),
        Point::new(-2896, -6957),
        Point::new(1883, -6948),
        Point::new(-7046, -4126),
        Point::new(-349, 3062),
        Point::new(-353, -5424),
        Point::new(-4558, 576),
        Point::new(7011, -3663),
        Point::new(-1920, -7422),
        Point::new(807, 5298),
        Point::new(1376, 690),
        Point::new(869, 315),
        Point::new(401, -3811),
        Point::new(1426, 5588),
        Point::new(3691, 2256),
        Point::new(29, 3161),
        Point::new(2013, 743),
        Point::new(-6429, -3975),
        Point::new(-1230, -8823),
        Point::new(3883, -5209),
        Point::new(-6297, -3984),
        Point::new(-7179, -4720),
        Point::new(1917, -1607),
        Point::new(2086, 3965),
        Point::new(-366, -2723),
        Point::new(-66, -3362),
        Point::new(3287, 874),
        Point::new(1776, -688),
        Point::new(965, 5727),
        Point::new(3232, 3345),
        Point::new(4786, -3386),
        Point::new(197, -4997),
        Point::new(2222, -2057),
        Point::new(-815, -4427),
        Point::new(-877, 4139),
        Point::new(791, 2413),
        Point::new(-2471, 2656),
        Point::new(5332, -4197),
        Point::new(1607, 2683),
        Point::new(4089, -4082),
        Point::new(5260, -449),
        Point::new(6545, -5347),
        Point::new(-6233, -1822),
        Point::new(1234, 6305),
        Point::new(-630, 5089),
        Point::new(-1814, -6803),
        Point::new(32, -3713),
        Point::new(1110, -3812),
        Point::new(-4007, -596),
        Point::new(1525, -530),
        Point::new(4270, -1077),
        Point::new(2799, 2485),
        Point::new(1234, 4713),
        Point::new(2654, -1968),
        Point::new(-1957, 4008),
        Point::new(-1324, 6528),
        Point::new(6466, -3358),
        Point::new(-1467, -8489),
        Point::new(1895, -5888),
        Point::new(-300, -4538),
        Point::new(1477, 2076),
        Point::new(318, 7783),
        Point::new(-4867, 398),
        Point::new(726, 6768),
        Point::new(-1380, 6602),
        Point::new(-2928, -7470),
        Point::new(1847, -8713),
        Point::new(795, 2949),
        Point::new(187, 1220),
        Point::new(723, 5018),
        Point::new(1783, -7888),
        Point::new(1774, -6870),
        Point::new(7045, -3238),
        Point::new(-4212, -94),
        Point::new(6846, -2831),
        Point::new(779, 4930),
        Point::new(1341, 3077),
        Point::new(-600, 7279),
        Point::new(-1013, -8044),
        Point::new(1782, 2445),
        Point::new(4898, -2241),
        Point::new(-3042, -7284),
        Point::new(-519, -5285),
        Point::new(-4594, -1822),
        Point::new(-1064, 4819),
        Point::new(1519, -6029),
        Point::new(1828, 514),
        Point::new(1499, -6868),
        Point::new(3784, -3679),
        Point::new(2208, -1741),
        Point::new(4898, -4178),
        Point::new(114, 4092),
        Point::new(5907, -4782),
        Point::new(-4021, 1626),
        Point::new(1672, -6121),
        Point::new(568, 2782),
        Point::new(-202, 8060),
        Point::new(-4995, -205),
        Point::new(3004, -8401),
        Point::new(433, -3110),
        Point::new(-1291, 3744),
        Point::new(4268, 1565),
        Point::new(-1790, -8508),
        Point::new(-1713, -6749),
        Point::new(4135, -1473),
        Point::new(-224, 3367),
        Point::new(-2181, -8798),
        Point::new(1465, 3565),
        Point::new(-177, 8202),
        Point::new(-1040, -7631),
        Point::new(5051, -4989),
        Point::new(751, 5110),
        Point::new(-7000, -4577),
        Point::new(3683, 1682),
        Point::new(3527, -1554),
        Point::new(6663, -3361),
        Point::new(2318, 3739),
        Point::new(3476, -1601),
        Point::new(634, -4613),
        Point::new(293, -4473),
        Point::new(4528, -5276),
        Point::new(-2387, 3390),
        Point::new(3560, 1623),
        Point::new(-1168, -4474),
        Point::new(-7307, -4417),
        Point::new(1870, 2294),
        Point::new(-2450, -6170),
        Point::new(1088, 5295),
        Point::new(2746, -994),
        Point::new(-6448, -4964),
        Point::new(1542, 4108),
        Point::new(1250, -3862),
        Point::new(2817, 3937),
        Point::new(306, 1301),
        Point::new(3971, -1041),
        Point::new(1855, -5211),
        Point::new(-1089, 6382),
        Point::new(-2981, 3027),
        Point::new(6583, -2481),
        Point::new(-3122, -8431),
        Point::new(-240, -4468),
        Point::new(-4541, -720),
        Point::new(1501, -7290),
        Point::new(754, -3967),
        Point::new(-1186, -8608),
        Point::new(-144, -5321),
        Point::new(641, -8341),
        Point::new(-191, -3372),
        Point::new(-2947, -8630),
        Point::new(543, 5232),
        Point::new(2430, -898),
        Point::new(1901, 3762),
        Point::new(265, -6683),
        Point::new(2283, -846),
        Point::new(-506, -8351),
        Point::new(-6439, -4045),
        Point::new(-639, 2729),
        Point::new(-1629, -6365),
        Point::new(-588, -6829),
        Point::new(5700, -3552),
        Point::new(-1808, -7796),
        Point::new(2495, 591),
        Point::new(145, -6367),
        Point::new(-1077, -4850),
        Point::new(5086, -380),
        Point::new(-276, 3590),
        Point::new(-1515, 5635),
        Point::new(998, 5340),
        Point::new(7166, -3941),
        Point::new(1137, -4129),
        Point::new(-6861, -4550),
        Point::new(3061, 2137),
        Point::new(-224, 4687),
        Point::new(2166, 3990),
        Point::new(1748, 723),
        Point::new(-102, 1751),
        Point::new(-7753, -5208),
        Point::new(2634, -3023),
        Point::new(-606, 7081),
        Point::new(4923, -582),
        Point::new(-737, -6511),
        Point::new(-3562, 1136),
        Point::new(3822, 1232),
        Point::new(-2148, -6649),
        Point::new(5498, -1745),
        Point::new(-1541, -6940),
        Point::new(4750, 764),
        Point::new(-2706, -7002),
        Point::new(5867, -4474),
        Point::new(-3360, -8553),
        Point::new(177, -3850),
        Point::new(2152, -6274),
        Point::new(6820, -3302),
        Point::new(-78, -6745),
        Point::new(11, -1765),
        Point::new(2608, -7834),
        Point::new(4104, -1969),
        Point::new(-2073, 4737),
        Point::new(2987, -3623),
        Point::new(4509, -729),
        Point::new(1279, 2314),
        Point::new(-4011, -567),
        Point::new(3606, 169),
        Point::new(1772, 1978),
        Point::new(3135, -2518),
        Point::new(-3469, 2361),
        Point::new(2242, 1215),
        Point::new(4119, -4125),
        Point::new(-7339, -4036),
        Point::new(-2221, 5199),
        Point::new(-2873, -7655),
        Point::new(-3177, 1859),
        Point::new(-18, 1674),
        Point::new(488, 7399),
        Point::new(6294, -5442),
        Point::new(682, 3089),
        Point::new(40, 3478),
        Point::new(6250, -3480),
        Point::new(977, 338),
        Point::new(4805, -1737),
        Point::new(-107, 7857),
        Point::new(-6580, -5038),
        Point::new(-2213, 4052),
        Point::new(-5776, -965),
        Point::new(1384, -5025),
        Point::new(3217, 1375),
        Point::new(5344, -993),
        Point::new(2492, 2326),
        Point::new(514, 7641),
        Point::new(-1803, -8150),
        Point::new(1015, 6559),
        Point::new(-6156, -4377),
        Point::new(-489, 7509),
        Point::new(3505, 791),
        Point::new(1808, -4942),
        Point::new(4600, -1254),
        Point::new(-818, -7688),
        Point::new(3667, -8776),
        Point::new(-3169, 3009),
        Point::new(-989, 6199),
        Point::new(156, -7138),
        Point::new(728, 1095),
        Point::new(-897, 4048),
        Point::new(308, -5832),
        Point::new(-3101, 2958),
        Point::new(2427, 4441),
        Point::new(1853, -8139),
        Point::new(2304, 615),
        Point::new(-423, 4574),
        Point::new(555, -7846),
        Point::new(-1768, -6478),
        Point::new(2883, -6807),
        Point::new(-1998, -7205),
        Point::new(-2122, -5962),
        Point::new(-793, 6173),
        Point::new(-388, 5110),
        Point::new(2788, -6782),
        Point::new(2302, -7826),
        Point::new(1901, 1435),
        Point::new(-68, -5193),
        Point::new(6710, -4634),
        Point::new(-2138, 3499),
        Point::new(-481, -4618),
        Point::new(-5504, -3241),
        Point::new(3700, -3080),
        Point::new(-5365, -3024),
        Point::new(4988, -5124),
        Point::new(4391, -639),
        Point::new(2706, -400),
        Point::new(-1969, 5067),
        Point::new(1675, -989),
        Point::new(4667, -5375),
        Point::new(54, -3910),
        Point::new(4585, -5141),
        Point::new(-1403, -6136),
        Point::new(3116, -502),
        Point::new(1027, -7593),
        Point::new(768, 5452),
        Point::new(648, 1808),
        Point::new(1586, -302),
        Point::new(1355, 3599),
        Point::new(-1094, -6768),
        Point::new(1078, -6025),
        Point::new(-809, 6114),
        Point::new(2046, 1986),
        Point::new(717, -5163),
        Point::new(522, 2405),
        Point::new(1672, -1033),
        Point::new(-3423, -8164),
        Point::new(-3766, 974),
        Point::new(3904, -1797),
        Point::new(-2069, -8617),
        Point::new(-2432, 1911),
        Point::new(-1375, -6354),
        Point::new(408, 7781),
        Point::new(-2837, 1709),
        Point::new(3613, 663),
        Point::new(2722, 3740),
        Point::new(1347, 6436),
        Point::new(4894, -5358),
        Point::new(-3586, 559),
        Point::new(924, 4951),
        Point::new(-1653, 4382),
        Point::new(-1895, -5407),
        Point::new(3564, -4529),
        Point::new(-5480, -1350),
        Point::new(3482, -2387),
        Point::new(-1305, -4918),
        Point::new(179, -5424),
        Point::new(846, 1570),
        Point::new(-83, 2412),
        Point::new(-4622, -564),
        Point::new(-5121, -732),
        Point::new(1033, -8254),
        Point::new(3046, 3067),
        Point::new(3124, -2771),
        Point::new(5627, -3517),
        Point::new(-1243, 5759),
        Point::new(-2357, 3119),
        Point::new(2506, -7072),
        Point::new(2239, -6930),
        Point::new(-1157, 4750),
        Point::new(5079, -3234),
        Point::new(6676, -3514),
        Point::new(-849, 5899),
        Point::new(-5061, -670),
        Point::new(5473, -2500),
        Point::new(3152, 850),
        Point::new(-2854, 1767),
        Point::new(-55, 4783),
        Point::new(192, 5948),
        Point::new(-1507, 5159),
        Point::new(576, 1883),
        Point::new(4870, -4966),
        Point::new(-7710, -5212),
        Point::new(3622, 892),
        Point::new(-987, -6005),
        Point::new(-1130, -4418),
        Point::new(-6503, -4758),
        Point::new(2051, -8611),
        Point::new(-2004, 3382),
        Point::new(3101, -3280),
        Point::new(-7093, -4121),
        Point::new(6611, -3501),
        Point::new(2149, -6739),
        Point::new(-586, -4604),
        Point::new(1530, 4297),
        Point::new(1464, 2830),
        Point::new(624, -8640),
        Point::new(4481, -1184),
        Point::new(-2842, 1868),
        Point::new(889, -3123),
        Point::new(3484, -1157),
        Point::new(2989, -8444),
        Point::new(5726, -3787),
        Point::new(-1658, -5832),
        Point::new(979, 2405),
        Point::new(1231, 6897),
        Point::new(3401, 1640),
        Point::new(2536, -753),
        Point::new(3465, -7968),
        Point::new(471, 3785),
        Point::new(-6467, -4118),
        Point::new(-2747, -8060),
        Point::new(-3532, 1828),
        Point::new(883, 2465),
        Point::new(-324, 6297),
        Point::new(-6284, -4165),
        Point::new(28, -1885),
        Point::new(-6892, -4912),
        Point::new(6012, -2562),
        Point::new(157, 5102),
        Point::new(-649, -7821),
        Point::new(1219, 4478),
        Point::new(3933, -3106),
        Point::new(216, -5849),
        Point::new(2336, -6581),
        Point::new(-1462, -7160),
        Point::new(4030, 375),
        Point::new(-997, -5217),
        Point::new(2740, -303),
        Point::new(2356, 130),
        Point::new(1196, -7764),
        Point::new(-2864, -7185),
        Point::new(4976, -1681),
        Point::new(2990, -2801),
        Point::new(2787, -2854),
        Point::new(1288, 1528),
        Point::new(964, -7466),
        Point::new(1509, -5922),
        Point::new(-237, -7023),
        Point::new(1564, 5313),
        Point::new(-6644, -4373),
        Point::new(1712, -8226),
        Point::new(4049, -2974),
        Point::new(1509, -6614),
        Point::new(5468, -4030),
        Point::new(-1631, 6150),
        Point::new(-949, 6027),
        Point::new(4296, 650),
        Point::new(-38, -4784),
        Point::new(-3912, -8749),
        Point::new(1547, 6034),
        Point::new(3102, 3014),
        Point::new(-2897, -7723),
        Point::new(-5331, -2103),
        Point::new(-5096, 192),
        Point::new(624, 3735),
        Point::new(-4247, -163),
        Point::new(-70, -6789),
        Point::new(-5258, -1775),
        Point::new(-3521, 585),
        Point::new(-310, -2439),
        Point::new(-6469, -3810),
        Point::new(1934, -5024),
        Point::new(420, 2595),
        Point::new(1479, -4493),
        Point::new(172, 2379),
        Point::new(6351, -4982),
        Point::new(2485, -7562),
        Point::new(3732, -1883),
        Point::new(-1748, -7850),
        Point::new(5379, -5419),
        Point::new(-2419, 4079),
        Point::new(3015, 2742),
        Point::new(-1243, -5700),
        Point::new(-5490, -2869),
        Point::new(-607, 5479),
        Point::new(-2042, 4287),
        Point::new(-1611, 3618),
        Point::new(-2427, 4299),
        Point::new(-3234, 923),
        Point::new(-2439, -7311),
        Point::new(-589, -3155),
        Point::new(-6210, -3469),
        Point::new(-475, 4548),
        Point::new(316, 5901),
        Point::new(6473, -3505),
        Point::new(4861, -3891),
        Point::new(-1968, -8301),
        Point::new(83, 4701),
        Point::new(-862, 4687),
        Point::new(2115, 3590),
        Point::new(663, 2238),
        Point::new(2850, -6572),
        Point::new(-3193, 3154),
        Point::new(-7476, -4890),
        Point::new(-3541, 1152),
        Point::new(-19, -6817),
        Point::new(326, -6583),
        Point::new(2520, -8764),
        Point::new(3996, -1952),
        Point::new(1177, 4703),
        Point::new(1417, -4922),
        Point::new(5766, -2838),
        Point::new(-1764, -6739),
        Point::new(5838, -1957),
        Point::new(5331, -2104),
        Point::new(-5613, -3588),
        Point::new(6862, -3127),
        Point::new(-6836, -4272),
        Point::new(-63, 2373),
        Point::new(4468, -471),
        Point::new(3350, -1876),
        Point::new(2150, 4656),
        Point::new(-5807, -1183),
        Point::new(-236, -3368),
        Point::new(-1293, 5081),
        Point::new(-5194, -2640),
        Point::new(6850, -3734),
        Point::new(-6045, -1641),
        Point::new(675, -4593),
        Point::new(-2978, 1297),
        Point::new(1913, 1542),
        Point::new(-3824, -107),
        Point::new(2595, 448),
        Point::new(3427, 393),
        Point::new(-327, 8093),
        Point::new(-5456, -1081),
        Point::new(6162, -2448),
        Point::new(-1113, 7058),
        Point::new(-5490, -1614),
        Point::new(2774, -6475),
        Point::new(-5819, -3482),
        Point::new(2464, 731),
        Point::new(-104, -7243),
        Point::new(7329, -4616),
        Point::new(3281, 1551),
        Point::new(3422, -1314),
        Point::new(-6740, -4135),
        Point::new(-1129, 5730),
        Point::new(4552, -2512),
        Point::new(2843, -6976),
        Point::new(7187, -4193),
        Point::new(3408, -3265),
        Point::new(1910, -4866),
        Point::new(-735, 7028),
        Point::new(2398, -7614),
        Point::new(-134, 2613),
        Point::new(1440, -8669),
        Point::new(4796, 68),
        Point::new(3060, -1428),
        Point::new(3440, -1202),
        Point::new(-318, -3641),
        Point::new(-592, -3968),
        Point::new(2814, -2245),
        Point::new(5347, -2877),
        Point::new(1285, -5400),
        Point::new(-5793, -2269),
        Point::new(6601, -2392),
        Point::new(-2295, -6587),
        Point::new(6056, -3325),
        Point::new(2352, 2082),
        Point::new(-3941, 879),
        Point::new(1823, 3190),
        Point::new(1613, -6949),
        Point::new(-446, -3570),
        Point::new(-472, -6649),
        Point::new(-4189, -532),
        Point::new(1566, 2869),
        Point::new(3311, 3047),
        Point::new(1166, -8565),
        Point::new(585, 1580),
        Point::new(1636, -4559),
        Point::new(4048, 1682),
        Point::new(-188, 5176),
        Point::new(-1530, -4674),
        Point::new(-46, -6214),
        Point::new(2497, -900),
        Point::new(6916, -3432),
        Point::new(-1850, -7487),
        Point::new(2919, -29),
        Point::new(2756, -6748),
        Point::new(1926, 4006),
        Point::new(3053, -892),
        Point::new(-2012, -7831),
        Point::new(-3102, 977),
        Point::new(-2405, 3161),
        Point::new(1551, -5882),
        Point::new(-5393, -1493),
        Point::new(-2992, -6935),
        Point::new(-1859, 5187),
        Point::new(-181, 8443),
        Point::new(1030, -5336),
        Point::new(2163, -8334),
        Point::new(-3684, 1780),
        Point::new(4302, -1399),
        Point::new(2087, 2021),
        Point::new(4983, -2088),
        Point::new(679, 380),
        Point::new(998, -8675),
        Point::new(2443, 1510),
        Point::new(-6551, -2799),
        Point::new(4968, -4268),
        Point::new(1373, 6307),
        Point::new(3138, -1523),
        Point::new(840, -7497),
        Point::new(-3259, 719),
        Point::new(-1947, -5081),
        Point::new(2528, 971),
        Point::new(-731, -4753),
        Point::new(1372, 895),
        Point::new(82, 7105),
        Point::new(3488, -2230),
        Point::new(38, -5236),
        Point::new(-3736, 1904),
        Point::new(5517, -3329),
        Point::new(1781, 1115),
        Point::new(370, -3618),
        Point::new(2824, -8366),
        Point::new(6205, -2287),
        Point::new(6342, -3325),
        Point::new(-769, -3976),
        Point::new(6392, -4878),
        Point::new(747, 1267),
        Point::new(265, 8534),
        Point::new(-7212, -5170),
        Point::new(2214, -8088),
        Point::new(3929, 3),
        Point::new(582, -4902),
        Point::new(415, -2854),
        Point::new(671, -6261),
        Point::new(2648, 660),
        Point::new(-3098, 2598),
        Point::new(-3863, -8693),
        Point::new(-509, -4192),
        Point::new(-20, 3305),
        Point::new(475, 4916),
        Point::new(4963, -1250),
        Point::new(7243, -5449),
        Point::new(-2291, 4932),
        Point::new(3334, -2811),
        Point::new(-1064, -7787),
        Point::new(-2974, -8563),
        Point::new(985, -5173),
        Point::new(3369, -8028),
        Point::new(1759, 766),
        Point::new(-2459, -6060),
        Point::new(2812, -849),
        Point::new(-6035, -4360),
        Point::new(-6979, -4881),
        Point::new(-252, -7357),
        Point::new(-2066, -7644),
        Point::new(75, 5143),
        Point::new(-2329, -6649),
        Point::new(-6612, -5069),
        Point::new(1545, -6256),
        Point::new(-1799, 3721),
        Point::new(4685, -1331),
        Point::new(-5643, -2970),
        Point::new(-2273, 3643),
        Point::new(-1156, -8817),
        Point::new(-6953, -3352),
        Point::new(6289, -3057),
        Point::new(4162, -4316),
        Point::new(983, 1203),
        Point::new(1799, 5510),
        Point::new(790, 2699),
        Point::new(-3, -6194),
        Point::new(-605, -3337),
        Point::new(-693, -5798),
        Point::new(-1887, 4164),
        Point::new(-4327, 1317),
        Point::new(2169, -7582),
        Point::new(892, 2778),
        Point::new(1679, -8438),
        Point::new(-2408, -6646),
        Point::new(5914, -4022),
        Point::new(3491, -8020),
        Point::new(2274, 4539),
        Point::new(5603, -2866),
        Point::new(-3419, 1915),
        Point::new(-444, 5654),
        Point::new(3014, 2031),
        Point::new(-173, -4771),
        Point::new(4034, -3561),
        Point::new(434, 8017),
        Point::new(-2996, 1194),
        Point::new(-1819, 4664),
        Point::new(2513, 2223),
        Point::new(2394, -6785),
        Point::new(-52, -2592),
        Point::new(-1970, -8139),
        Point::new(3117, -2951),
        Point::new(4557, -473),
        Point::new(1488, 1514),
        Point::new(-749, 7458),
        Point::new(-6137, -3751),
        Point::new(4306, -4467),
        Point::new(6080, -3779),
        Point::new(-2631, -7353),
        Point::new(-6416, -3931),
        Point::new(-4818, -1713),
        Point::new(105, -2663),
        Point::new(-1204, -6854),
        Point::new(-5116, -865),
        Point::new(-2153, -8288),
        Point::new(-4004, -768),
        Point::new(2998, -1247),
        Point::new(4105, -4549),
        Point::new(-2661, -6968),
        Point::new(5869, -2163),
        Point::new(4928, -5395),
        Point::new(-5184, -1856),
        Point::new(4845, -2657),
        Point::new(2568, 2906),
        Point::new(-7678, -5301),
        Point::new(3572, -105),
        Point::new(-1397, -5973),
        Point::new(241, 7849),
        Point::new(993, -4739),
        Point::new(766, 514),
        Point::new(-5292, -1628),
        Point::new(2335, -136),
        Point::new(804, 4448),
        Point::new(-2524, -7827),
        Point::new(-1003, 4036),
        Point::new(-994, -4586),
        Point::new(-1672, 4297),
        Point::new(6310, -2645),
        Point::new(1441, -611),
        Point::new(5974, -5400),
        Point::new(5726, -5025),
        Point::new(607, 1497),
        Point::new(-334, -6012),
        Point::new(-5668, -3768),
        Point::new(295, 7486),
        Point::new(-1735, 5473),
        Point::new(-3091, -8034),
        Point::new(6310, -5377),
        Point::new(1303, -4556),
        Point::new(-3210, -7814),
        Point::new(-6397, -3031),
        Point::new(1739, 5991),
        Point::new(24, -4383),
        Point::new(1788, -138),
        Point::new(2739, -535),
        Point::new(3085, -512),
        Point::new(2645, -7670),
        Point::new(-7496, -4489),
        Point::new(2796, -7884),
        Point::new(2094, 1553),
        Point::new(-1188, -5890),
        Point::new(3560, -272),
        Point::new(5082, -1825),
        Point::new(-443, -5183),
        Point::new(1544, 2555),
        Point::new(-1068, 6622),
        Point::new(6099, -1613),
        Point::new(444, -5733),
        Point::new(5007, -3537),
        Point::new(-5742, -1964),
        Point::new(1587, -245),
        Point::new(-273, -7642),
        Point::new(1393, 3615),
        Point::new(336, 2663),
        Point::new(-725, 5526),
        Point::new(78, 1572),
        Point::new(1967, -6502),
        Point::new(1848, 3591),
        Point::new(-7719, -4848),
        Point::new(3160, -1232),
        Point::new(1913, -8604),
        Point::new(-72, 2935),
        Point::new(-918, -7170),
        Point::new(1708, -1012),
        Point::new(845, 5820),
        Point::new(3892, 1435),
        Point::new(-2687, 4013),
        Point::new(3742, -4003),
        Point::new(2695, -1547),
        Point::new(1425, 1360),
        Point::new(3011, 1418),
        Point::new(6719, -3961),
        Point::new(-1247, -5735),
        Point::new(3404, -2618),
        Point::new(-4252, -734),
        Point::new(6624, -5518),
        Point::new(2726, 2503),
        Point::new(492, 8076),
        Point::new(4768, -3874),
        Point::new(4295, 809),
        Point::new(4484, -4737),
        Point::new(5833, -2300),
        Point::new(-3437, 2552),
        Point::new(-312, 2217),
        Point::new(1287, 2728),
        Point::new(-3071, 2776),
        Point::new(4094, -2138),
        Point::new(5870, -3497),
        Point::new(5188, -587),
        Point::new(5938, -2771),
        Point::new(-5190, -1629),
        Point::new(1109, -7463),
        Point::new(-7369, -4233),
        Point::new(-1595, -5966),
        Point::new(-1951, 3477),
        Point::new(6868, -2881),
        Point::new(-2682, 4299),
        Point::new(-6767, -5484),
        Point::new(3591, -4538),
        Point::new(-6295, -3029),
        Point::new(-6474, -2409),
        Point::new(675, 1909),
        Point::new(-1288, -3968),
        Point::new(-5925, -1279),
        Point::new(-1323, 6218),
        Point::new(-592, 3028),
        Point::new(4291, 1551),
        Point::new(6195, -3364),
        Point::new(2527, 4532),
        Point::new(2034, 1911),
        Point::new(5812, -2129),
        Point::new(1148, -3753),
        Point::new(4548, -86),
        Point::new(-2598, 4412),
        Point::new(2330, -6438),
        Point::new(5488, -1091),
        Point::new(-292, 3072),
        Point::new(298, 6338),
        Point::new(-2351, -7130),
        Point::new(-1350, 3846),
        Point::new(-4515, 811),
        Point::new(2611, -1694),
        Point::new(1815, 1462),
        Point::new(2806, -1425),
        Point::new(-201, -2981),
        Point::new(-524, 3171),
        Point::new(1166, -499),
        Point::new(5848, -2746),
        Point::new(256, 5666),
        Point::new(7492, -5394),
        Point::new(-939, 6282),
        Point::new(2490, 3282),
        Point::new(185, 4362),
        Point::new(758, 5409),
        Point::new(-176, 5759),
        Point::new(7064, -5277),
        Point::new(6382, -5083),
        Point::new(-5773, -2886),
        Point::new(6354, -3146),
        Point::new(503, 5034),
        Point::new(1268, 3368),
        Point::new(-4027, -558),
        Point::new(2378, 1130),
        Point::new(291, 3829),
        Point::new(3023, -214),
        Point::new(5653, -1296),
        Point::new(-6549, -5130),
        Point::new(5775, -4542),
        Point::new(4061, -2562),
        Point::new(-895, -3963),
        Point::new(-82, -2154),
        Point::new(1728, -8416),
        Point::new(178, -8609),
        Point::new(-6443, -2180),
        Point::new(5859, -2047),
        Point::new(-1406, 4479),
        Point::new(-973, -3970),
        Point::new(4831, 326),
        Point::new(-6363, -3849),
        Point::new(4570, 1124),
        Point::new(6700, -4941),
        Point::new(-1788, 3183),
        Point::new(2320, 3304),
        Point::new(5103, -1625),
        Point::new(173, 8347),
        Point::new(-6429, -4868),
        Point::new(3772, -2700),
        Point::new(1304, 526),
        Point::new(5644, -4327),
        Point::new(2229, 651),
        Point::new(2983, 3563),
        Point::new(-3471, 2364),
        Point::new(-1115, -6753),
        Point::new(1531, -7997),
        Point::new(6190, -5432),
        Point::new(1356, -512),
        Point::new(-5731, -3141),
        Point::new(6931, -4820),
        Point::new(-6044, -4127),
        Point::new(344, 6183),
        Point::new(9, -3906),
        Point::new(1868, -6196),
        Point::new(4183, -1710),
        Point::new(-1150, 6035),
        Point::new(1696, 5696),
        Point::new(-1329, -6791),
        Point::new(2215, -169),
        Point::new(3660, -3292),
        Point::new(-2407, 2053),
        Point::new(3112, 359),
        Point::new(-1135, -6933),
        Point::new(-428, -3432),
        Point::new(3453, 2771),
        Point::new(-3168, -7781),
        Point::new(867, 5902),
        Point::new(146, -6265),
        Point::new(1012, -4215),
        Point::new(6027, -2575),
        Point::new(-1345, -4024),
        Point::new(-6775, -4187),
        Point::new(3529, 146),
        Point::new(-2453, -8082),
        Point::new(3678, -2860),
        Point::new(4354, -4808),
        Point::new(308, -7166),
        Point::new(3148, -602),
        Point::new(-404, 6699),
        Point::new(-5150, -2172),
        Point::new(2083, 4295),
        Point::new(-748, 3099),
        Point::new(2613, 3337),
        Point::new(3446, -4393),
        Point::new(-6506, -4601),
        Point::new(2842, 1628),
        Point::new(2544, -483),
        Point::new(4385, -2561),
        Point::new(1232, -8669),
        Point::new(-5445, -438),
        Point::new(2040, 5478),
        Point::new(4564, -358),
        Point::new(2057, -99),
        Point::new(-5890, -3424),
        Point::new(787, 3447),
        Point::new(-22, -8191),
        Point::new(-2834, -8065),
        Point::new(-371, -7110),
        Point::new(2343, 3449),
        Point::new(4652, -69),
        Point::new(-1799, 3576),
        Point::new(3821, -246),
        Point::new(4354, -969),
        Point::new(7352, -4775),
        Point::new(-5764, -2051),
        Point::new(819, 894),
        Point::new(-6207, -3644),
        Point::new(1207, -3955),
        Point::new(-420, 2856),
        Point::new(-1007, 5105),
        Point::new(-99, -3312),
        Point::new(-3476, 1981),
        Point::new(101, -3299),
        Point::new(-1281, 4472),
        Point::new(1892, 237),
        Point::new(-750, -5815),
        Point::new(375, -8357),
        Point::new(5550, -4329),
        Point::new(5866, -1513),
        Point::new(4139, -3908),
        Point::new(3728, 1462),
        Point::new(-3065, -8509),
        Point::new(3695, -2942),
        Point::new(4181, -1441),
        Point::new(4378, 677),
        Point::new(133, -7201),
        Point::new(-1695, -8496),
        Point::new(3039, -3392),
        Point::new(4879, -2541),
        Point::new(-167, -8185),
        Point::new(-2239, -7687),
        Point::new(-4696, -1315),
        Point::new(659, 6467),
        Point::new(1162, -4637),
        Point::new(1634, 2993),
        Point::new(6082, -3536),
        Point::new(1215, -5422),
        Point::new(-2487, 4020),
        Point::new(-5944, -4118),
        Point::new(340, -4926),
        Point::new(-5906, -2668),
        Point::new(466, -8515),
        Point::new(6593, -4840),
        Point::new(980, -5302),
        Point::new(1059, 4189),
        Point::new(2813, -1307),
        Point::new(2478, -8694),
        Point::new(2693, -6983),
        Point::new(-6997, -3949),
        Point::new(1444, -8313),
        Point::new(365, 5239),
        Point::new(-6399, -3909),
        Point::new(2454, 943),
        Point::new(-389, 5908),
        Point::new(1572, 2444),
        Point::new(1096, -7681),
        Point::new(-792, -8116),
        Point::new(2254, -8266),
        Point::new(-95, 8324),
        Point::new(-294, -6780),
        Point::new(23, -1944),
        Point::new(-256, -7624),
        Point::new(-1131, 5703),
        Point::new(-194, 6323),
        Point::new(-1650, 5155),
        Point::new(-1446, 4611),
        Point::new(-3286, -8731),
        Point::new(872, -5932),
        Point::new(2284, 352),
        Point::new(-4688, 629),
        Point::new(-694, -4432),
        Point::new(2516, -693),
        Point::new(-1338, -6821),
        Point::new(562, -3962),
        Point::new(93, -2992),
        Point::new(1223, -7908),
        Point::new(5444, -1449),
        Point::new(3489, -3461),
        Point::new(3174, 2495),
        Point::new(3129, -1800),
        Point::new(-1211, 4366),
        Point::new(7328, -4604),
        Point::new(-7040, -4733),
        Point::new(1621, 4912),
        Point::new(1280, 4733),
        Point::new(7217, -4560),
        Point::new(3716, -3160),
        Point::new(-1639, 4621),
        Point::new(1650, 5621),
        Point::new(-593, 3628),
        Point::new(901, -7978),
        Point::new(3797, -73),
        Point::new(4298, -2700),
        Point::new(-5895, -3888),
        Point::new(2273, -7937),
        Point::new(-8100, -5178),
        Point::new(-1005, 6789),
        Point::new(7151, -3735),
        Point::new(2981, -6921),
        Point::new(7155, -3487),
        Point::new(-286, -5972),
        Point::new(1612, -5121),
        Point::new(-6668, -3955),
        Point::new(480, 6745),
        Point::new(1482, 5521),
        Point::new(-7812, -5501),
        Point::new(3635, -8727),
        Point::new(2141, 4987),
        Point::new(669, 3039),
        Point::new(-1133, 6894),
        Point::new(3790, -4889),
        Point::new(-1325, -4267),
        Point::new(-1482, -7107),
        Point::new(-2666, -7051),
        Point::new(586, -4511),
        Point::new(-1131, -6204),
        Point::new(-1936, -7325),
        Point::new(1636, -450),
        Point::new(2723, -2399),
        Point::new(6029, -1824),
        Point::new(-7099, -4971),
        Point::new(2470, 379),
        Point::new(3289, 2620),
        Point::new(3857, -8771),
        Point::new(-2792, 1599),
        Point::new(-6138, -3655),
        Point::new(351, 5522),
        Point::new(3886, 1584),
        Point::new(1977, -8648),
        Point::new(2799, 2999),
        Point::new(74, -6006),
        Point::new(-880, -7186),
        Point::new(1842, -7717),
        Point::new(-2624, -8825),
        Point::new(1807, -1542),
        Point::new(374, 5611),
        Point::new(5964, -5410),
        Point::new(189, 6606),
        Point::new(3154, -1370),
        Point::new(693, 4983),
        Point::new(1628, -6369),
        Point::new(2576, 3768),
        Point::new(2055, 5314),
        Point::new(5021, -1301),
        Point::new(662, 1778),
        Point::new(2517, 4326),
        Point::new(1162, -6328),
        Point::new(6352, -4242),
        Point::new(7974, -5397),
        Point::new(-2397, -8699),
        Point::new(1688, 4049),
        Point::new(-3494, -8801),
        Point::new(7722, -4461),
        Point::new(-923, -5268),
        Point::new(-4023, 1729),
        Point::new(3461, -1492),
        Point::new(2409, -7477),
        Point::new(1932, -5519),
        Point::new(5116, -2784),
        Point::new(-4248, 1182),
        Point::new(3484, -3866),
        Point::new(222, 1212),
        Point::new(3825, -2052),
        Point::new(1248, 967),
        Point::new(1841, 1293),
        Point::new(820, -3374),
        Point::new(1178, -6671),
        Point::new(4056, 773),
        Point::new(2922, -1244),
        Point::new(-738, 5421),
        Point::new(-1120, -6285),
        Point::new(5057, -930),
        Point::new(3740, -8479),
        Point::new(3507, -1652),
        Point::new(-1831, -7467),
        Point::new(1215, 4988),
        Point::new(23, 7886),
        Point::new(5253, -3682),
        Point::new(1850, -1611),
        Point::new(4065, -4577),
        Point::new(395, -5258),
        Point::new(-3627, -8013),
        Point::new(-285, -4672),
        Point::new(3148, 993),
        Point::new(6738, -3886),
        Point::new(3596, 1494),
        Point::new(1971, 1355),
        Point::new(-541, 7211),
        Point::new(599, -3745),
        Point::new(-7395, -5099),
        Point::new(2482, 4203),
        Point::new(-1142, 5462),
        Point::new(391, 2994),
        Point::new(1476, 4826),
        Point::new(-2054, -7479),
        Point::new(-833, -4917),
        Point::new(3376, -699),
        Point::new(2518, 2977),
        Point::new(2966, -1074),
        Point::new(-1339, -5048),
        Point::new(3709, -2394),
        Point::new(3645, 150),
        Point::new(-620, -5295),
        Point::new(-1564, -6737),
        Point::new(475, 2165),
        Point::new(1929, -1203),
        Point::new(880, 1905),
        Point::new(-354, -5551),
        Point::new(-1162, -7841),
        Point::new(2448, 2741),
        Point::new(-3313, -7353),
        Point::new(2243, -1793),
        Point::new(-1248, -6900),
        Point::new(1623, -6799),
        Point::new(5791, -1033),
        Point::new(1086, -4956),
        Point::new(2137, 1068),
        Point::new(25, 3600),
        Point::new(3569, -7765),
        Point::new(-2442, -8528),
        Point::new(6178, -4121),
        Point::new(-1771, -8764),
        Point::new(-3528, -7881),
        Point::new(1505, 890),
        Point::new(-214, -7638),
        Point::new(7210, -3453),
        Point::new(-2053, -8112),
        Point::new(-502, -5937),
        Point::new(3489, 2127),
        Point::new(-2521, 1876),
        Point::new(-6045, -1977),
        Point::new(-1114, 5390),
        Point::new(2051, 2163),
        Point::new(1275, 497),
        Point::new(2389, -2024),
        Point::new(-245, 8384),
        Point::new(4441, 410),
        Point::new(-6466, -3180),
        Point::new(4447, -3019),
        Point::new(3860, -2610),
        Point::new(604, -4725),
        Point::new(7434, -4829),
        Point::new(78, -6806),
        Point::new(228, -8228),
        Point::new(-7207, -4040),
        Point::new(682, -8186),
        Point::new(2678, -1101),
        Point::new(-1329, -7513),
        Point::new(783, -7017),
        Point::new(-1127, -6502),
        Point::new(7764, -5302),
        Point::new(480, 5955),
        Point::new(4634, -3041),
        Point::new(1747, 4798),
        Point::new(-5241, -1107),
        Point::new(-1789, -6405),
        Point::new(-1914, -8757),
        Point::new(2662, 1163),
        Point::new(1841, 5606),
        Point::new(4879, 188),
        Point::new(3194, -8345),
        Point::new(3078, -8816),
        Point::new(-3173, 3489),
        Point::new(4260, -4956),
        Point::new(4613, 40),
        Point::new(3896, 1250),
        Point::new(5709, -5261),
        Point::new(1582, 4936),
        Point::new(3061, -8007),
        Point::new(-7410, -4331),
        Point::new(-7646, -4423),
        Point::new(322, -5877),
        Point::new(3434, -3096),
        Point::new(3205, -8379),
        Point::new(-2130, -8203),
        Point::new(-2438, 2196),
        Point::new(2039, 62),
        Point::new(1889, -5687),
        Point::new(-1295, 5520),
        Point::new(525, -2634),
        Point::new(7093, -3406),
        Point::new(-1166, -8250),
        Point::new(1798, -5016),
        Point::new(7153, -3787),
        Point::new(1640, -8809),
        Point::new(1266, 1745),
        Point::new(-1561, 3682),
        Point::new(389, 4110),
        Point::new(974, -6534),
        Point::new(-5832, -2074),
        Point::new(-7278, -5251),
        Point::new(3576, 2223),
        Point::new(1589, 3840),
        Point::new(-5723, -2824),
        Point::new(6703, -4619),
        Point::new(1301, -8149),
        Point::new(2057, 17),
        Point::new(-4029, 1699),
        Point::new(1992, -47),
        Point::new(-1469, -4234),
        Point::new(-2316, -7413),
        Point::new(2599, 4294),
        Point::new(2556, -8618),
        Point::new(2689, 3741),
        Point::new(-127, 2802),
        Point::new(-941, -8185),
        Point::new(-1355, 5632),
        Point::new(-101, 4694),
        Point::new(1890, 4245),
        Point::new(-5579, -613),
        Point::new(-691, -4331),
        Point::new(2746, -311),
        Point::new(4417, 604),
        Point::new(-690, -6281),
        Point::new(-4427, -393),
        Point::new(2696, -7057),
        Point::new(4804, -4042),
        Point::new(4350, -3395),
        Point::new(-1057, -6288),
        Point::new(1842, -6395),
        Point::new(3705, -8686),
        Point::new(-370, 3483),
        Point::new(-755, 6606),
        Point::new(-6418, -3674),
        Point::new(-946, -5276),
        Point::new(-43, -4633),
        Point::new(-692, 5955),
        Point::new(-3285, 2500),
        Point::new(-3696, -8377),
        Point::new(-4984, -1322),
        Point::new(-7301, -5186),
        Point::new(34, 4273),
        Point::new(451, 5179),
        Point::new(1793, -7234),
        Point::new(2047, 1364),
        Point::new(6055, -3991),
        Point::new(-4332, -747),
        Point::new(2998, -3021),
        Point::new(1038, -4601),
        Point::new(2007, -1739),
        Point::new(-3716, 1301),
        Point::new(-33, 8824),
        Point::new(-131, -6941),
        Point::new(-7234, -5185),
        Point::new(2926, -8820),
        Point::new(887, -3390),
        Point::new(3654, -8525),
        Point::new(6993, -5293),
        Point::new(4761, -4326),
        Point::new(-799, -5688),
        Point::new(1499, -8547),
        Point::new(-1218, 4737),
        Point::new(5962, -5129),
        Point::new(2957, -2793),
        Point::new(-2800, -8557),
        Point::new(2124, -456),
        Point::new(5596, -1711),
        Point::new(-7107, -3249),
        Point::new(3488, 2349),
        Point::new(184, 2294),
        Point::new(1749, -6636),
        Point::new(-28, -4127),
        Point::new(2383, 3562),
        Point::new(2714, -2029),
        Point::new(761, 5982),
        Point::new(2987, 3018),
        Point::new(-529, -3656),
        Point::new(3255, -8744),
        Point::new(4462, -1647),
        Point::new(-6600, -3480),
        Point::new(-591, -3580),
        Point::new(2612, -8592),
        Point::new(2055, 3368),
        Point::new(4293, -5498),
        Point::new(-540, -3950),
        Point::new(-6056, -2538),
        Point::new(6972, -3879),
        Point::new(277, -2618),
        Point::new(3914, -182),
        Point::new(484, -5532),
        Point::new(-3025, 1399),
        Point::new(-679, 4419),
        Point::new(2882, 4070),
        Point::new(1216, -5796),
        Point::new(6753, -4095),
        Point::new(3900, -1451),
        Point::new(3801, 932),
        Point::new(2452, -1956),
        Point::new(7301, -5230),
        Point::new(-3685, -8453),
        Point::new(-748, 4295),
        Point::new(-1536, -4574),
        Point::new(3239, -2843),
        Point::new(-2039, -7816),
        Point::new(-5837, -2304),
        Point::new(-2458, -7105),
        Point::new(3039, -312),
        Point::new(2734, 2193),
        Point::new(2526, 1993),
        Point::new(-1982, 4473),
        Point::new(-2674, 2803),
        Point::new(-1406, -8124),
        Point::new(2190, 2806),
        Point::new(45, -3193),
        Point::new(-5734, -1915),
        Point::new(-3945, 854),
        Point::new(3047, -7658),
        Point::new(6473, -4631),
        Point::new(2028, -7336),
        Point::new(-2376, 2151),
        Point::new(499, 6935),
        Point::new(2587, 1172),
        Point::new(-5164, -1394),
        Point::new(4005, 1494),
        Point::new(1926, -134),
        Point::new(1386, 5105),
        Point::new(842, -6351),
        Point::new(2866, -2653),
        Point::new(7696, -5204),
        Point::new(4895, 495),
        Point::new(1998, -1651),
        Point::new(295, 5231),
        Point::new(226, 4462),
        Point::new(-212, -2635),
        Point::new(1171, -6438),
        Point::new(389, 913),
        Point::new(-511, -8672),
        Point::new(5520, -3577),
        Point::new(4600, -3677),
        Point::new(781, -6737),
        Point::new(-5601, -2156),
        Point::new(6098, -5412),
        Point::new(-961, 6069),
        Point::new(3647, 35),
        Point::new(-6349, -3054),
        Point::new(-958, -6994),
        Point::new(1222, 2127),
        Point::new(324, 2384),
        Point::new(-5974, -1596),
        Point::new(-2522, 3146),
        Point::new(1332, 6581),
        Point::new(2154, -6229),
        Point::new(-6667, -3698),
        Point::new(-1869, 4668),
        Point::new(6817, -2841),
        Point::new(4160, -2789),
        Point::new(2025, -5320),
        Point::new(1255, 3097),
        Point::new(218, -5891),
        Point::new(6588, -3242),
        Point::new(-2316, -5862),
        Point::new(3822, -8692),
        Point::new(-236, -6765),
        Point::new(151, 7700),
        Point::new(-324, 3044),
        Point::new(7662, -5010),
        Point::new(977, 2954),
        Point::new(-8141, -1090),
        Point::new(5770, 276),
        Point::new(7238, 607),
        Point::new(681, 6908),
        Point::new(-4835, 1484),
        Point::new(4181, 3148),
        Point::new(-5279, -3375),
        Point::new(7043, 5800),
        Point::new(2982, -7898),
        Point::new(-1960, 7661),
        Point::new(4105, 5673),
        Point::new(-7225, -3970),
        Point::new(-5979, -7191),
        Point::new(3871, -5782),
        Point::new(-4279, 2784),
        Point::new(4474, 5752),
        Point::new(314, 2112),
        Point::new(3355, 7823),
        Point::new(-7569, 4754),
        Point::new(-5744, -425),
        Point::new(-3604, 1981),
        Point::new(4280, 8480),
        Point::new(3852, 7169),
        Point::new(2835, 1653),
        Point::new(-7439, 7046),
        Point::new(-1650, -4892),
        Point::new(7479, 7150),
        Point::new(5502, 1622),
        Point::new(7069, 7545),
        Point::new(5824, 2842),
        Point::new(-1945, 6411),
        Point::new(-2418, 4396),
        Point::new(-6486, -6534),
        Point::new(6697, 5504),
        Point::new(30, -7409),
        Point::new(312, -5382),
        Point::new(177, -5767),
        Point::new(7477, -5418),
        Point::new(7925, 2817),
        Point::new(356, -7121),
        Point::new(-3816, -548),
        Point::new(6716, 5503),
        Point::new(5717, -7192),
        Point::new(-2085, 8550),
        Point::new(-4723, 2336),
        Point::new(-6267, -2391),
        Point::new(4405, -3671),
        Point::new(-2193, -1886),
        Point::new(-5839, 8495),
        Point::new(3967, -5750),
        Point::new(2103, 3632),
        Point::new(-5076, -903),
        Point::new(8230, 3242),
        Point::new(6229, -7057),
        Point::new(-4057, 3514),
        Point::new(8039, 2791),
        Point::new(-5800, 8369),
        Point::new(578, 6951),
        Point::new(-2918, 3299),
        Point::new(-7503, 495),
        Point::new(77, -5029),
        Point::new(4903, -3657),
        Point::new(-7280, 7742),
        Point::new(610, 7394),
        Point::new(-8085, -2558),
        Point::new(4358, -3562),
        Point::new(-1378, -8291),
        Point::new(-289, -5712),
        Point::new(7600, 718),
        Point::new(4604, -3601),
        Point::new(3113, -7405),
        Point::new(-3402, 115),
        Point::new(2845, -1948),
        Point::new(1771, 3277),
        Point::new(1900, 3737),
        Point::new(-1943, 6648),
        Point::new(-485, 2692),
        Point::new(3748, -5796),
        Point::new(6863, -3664),
        Point::new(6158, -2701),
        Point::new(2220, -3424),
        Point::new(-4790, 4921),
        Point::new(4201, 7836),
        Point::new(537, 7407),
        Point::new(-411, -4775),
        Point::new(686, 7273),
        Point::new(-1203, -6403),
        Point::new(-3697, -356),
        Point::new(-1149, 2476),
        Point::new(928, -3258),
        Point::new(-3320, -217),
        Point::new(-2805, -4123),
        Point::new(3201, 2403),
        Point::new(8461, 3396),
        Point::new(7515, -3005),
        Point::new(1901, 1916),
        Point::new(4383, 6007),
        Point::new(3582, -3474),
        Point::new(1917, 4508),
        Point::new(3582, -5917),
        Point::new(8316, 213),
        Point::new(-2908, 2255),
        Point::new(6426, -3355),
        Point::new(-6083, -4432),
        Point::new(1169, 6860),
        Point::new(-7703, 3757),
        Point::new(7492, 3620),
        Point::new(5378, 1996),
        Point::new(3043, -3108),
        Point::new(1528, 3398),
        Point::new(-9473, -4653),
        Point::new(3967, -6080),
        Point::new(2004, 3679),
        Point::new(7759, -5595),
        Point::new(-3052, -251),
        Point::new(2381, -2127),
        Point::new(283, 105),
        Point::new(-3664, 2419),
        Point::new(1377, 3545),
        Point::new(5713, 853),
        Point::new(6417, -3328),
        Point::new(-407, 1560),
        Point::new(-2063, -3822),
        Point::new(-5347, -4509),
        Point::new(4863, -3989),
        Point::new(6255, 438),
        Point::new(-2459, 7365),
        Point::new(4333, 6148),
        Point::new(6182, 555),
        Point::new(7453, 59),
        Point::new(1762, -3037),
        Point::new(-8333, 7068),
        Point::new(-1919, -4788),
        Point::new(8923, -720),
        Point::new(7006, -3428),
        Point::new(2334, 3954),
        Point::new(2759, -7458),
        Point::new(4591, 5361),
        Point::new(7856, 3503),
        Point::new(1631, 3809),
        Point::new(5451, 786),
        Point::new(646, 7431),
        Point::new(-8176, 7175),
        Point::new(-2741, -2081),
        Point::new(-1457, 7707),
        Point::new(804, 1086),
        Point::new(3022, -7502),
        Point::new(-5161, -793),
        Point::new(2892, -7694),
        Point::new(6810, -3655),
        Point::new(-5794, 4079),
        Point::new(1825, 4044),
        Point::new(-5309, 4917),
        Point::new(3926, -5752),
        Point::new(-1349, -5572),
        Point::new(-2755, -1955),
        Point::new(6754, -5749),
        Point::new(3376, -7771),
        Point::new(-218, 543),
        Point::new(-3782, 3207),
        Point::new(-2296, -1788),
        Point::new(-1607, 7496),
        Point::new(4353, 5544),
        Point::new(162, -6163),
        Point::new(7286, 7481),
        Point::new(6637, 5072),
        Point::new(-2359, 7002),
        Point::new(-5820, -2159),
        Point::new(492, 486),
        Point::new(-6804, -1019),
        Point::new(-1255, -5027),
        Point::new(-2309, -1793),
        Point::new(-3465, -453),
        Point::new(5575, 263),
        Point::new(-2985, 2076),
        Point::new(5864, 754),
        Point::new(-6482, 3570),
        Point::new(6459, 5302),
        Point::new(423, -6399),
        Point::new(-1930, -7347),
        Point::new(-6813, 1366),
        Point::new(8190, 967),
        Point::new(-8139, 7705),
        Point::new(2130, -4505),
        Point::new(-9206, -4309),
        Point::new(-2439, -7341),
        Point::new(561, 7157),
        Point::new(7596, 3458),
        Point::new(-457, -5197),
        Point::new(8274, -5353),
        Point::new(8009, -5569),
        Point::new(4395, 561),
        Point::new(8130, 335),
        Point::new(7023, 8171),
        Point::new(-5503, 1279),
        Point::new(297, -7422),
        Point::new(7730, 3753),
        Point::new(-2529, 7481),
        Point::new(-4195, -4148),
        Point::new(4840, 357),
        Point::new(4472, 5758),
        Point::new(4156, 5054),
        Point::new(-5914, -6582),
        Point::new(-5379, -919),
        Point::new(3875, -5850),
        Point::new(-4691, 2065),
        Point::new(-3054, 4882),
        Point::new(1520, 3557),
        Point::new(-2074, -7548),
        Point::new(7759, 3631),
        Point::new(-1547, 4123),
        Point::new(6508, -2998),
        Point::new(-4006, -698),
        Point::new(-6015, 7033),
        Point::new(4755, 6247),
        Point::new(8236, 763),
        Point::new(7554, 3137),
        Point::new(4962, 1852),
        Point::new(-421, -5637),
        Point::new(-1638, 7872),
        Point::new(1768, -1770),
        Point::new(-677, -6818),
        Point::new(-3316, -342),
        Point::new(1388, -2134),
        Point::new(240, -7369),
        Point::new(-7861, -454),
        Point::new(-7571, -4297),
        Point::new(7214, 7537),
        Point::new(6876, 7447),
        Point::new(-7202, -1692),
        Point::new(-2434, -3051),
        Point::new(-645, 3327),
        Point::new(1534, 3908),
        Point::new(-1727, 7748),
        Point::new(-4795, 1858),
        Point::new(-6036, -6426),
        Point::new(8293, 3468),
        Point::new(7925, -3617),
        Point::new(2964, -7587),
        Point::new(-9024, -4615),
        Point::new(-3440, -6613),
        Point::new(2860, -2424),
        Point::new(212, -5802),
        Point::new(6232, 726),
        Point::new(706, -3623),
        Point::new(4234, 1131),
        Point::new(-6321, -7027),
        Point::new(-1141, -3971),
        Point::new(8225, 1108),
        Point::new(-1924, -1547),
        Point::new(7763, 795),
        Point::new(-5198, -854),
        Point::new(-4322, 7774),
        Point::new(-7848, 7454),
        Point::new(5406, 485),
        Point::new(5820, -7369),
        Point::new(-4646, 7520),
        Point::new(-4671, 1401),
        Point::new(6403, 3861),
        Point::new(-5644, -3476),
        Point::new(-876, -5049),
        Point::new(-7669, 4698),
        Point::new(5225, 44),
        Point::new(-1993, -2860),
        Point::new(-1657, 2235),
        Point::new(7542, -502),
        Point::new(6923, 7800),
        Point::new(3790, -8062),
        Point::new(-5429, -2031),
        Point::new(-2172, -1010),
        Point::new(4067, -193),
        Point::new(7050, -404),
        Point::new(8045, 317),
        Point::new(-8726, 6798),
        Point::new(-7021, 2136),
        Point::new(-7719, 7829),
        Point::new(-5190, -785),
        Point::new(-269, 1264),
        Point::new(4080, -5709),
        Point::new(6657, 5582),
        Point::new(-7145, 3590),
        Point::new(-1865, 6972),
        Point::new(4693, -3990),
        Point::new(308, -7489),
        Point::new(-863, -6304),
        Point::new(5804, -7540),
        Point::new(4461, 5789),
        Point::new(7237, -2913),
        Point::new(-172, -5248),
        Point::new(-2515, 8183),
        Point::new(-8627, -3781),
        Point::new(8299, 2931),
        Point::new(2862, -7958),
        Point::new(-7033, 8233),
        Point::new(1460, 393),
        Point::new(-2303, 6634),
        Point::new(-1794, 8320),
        Point::new(-5871, 6503),
        Point::new(-6563, 6859),
        Point::new(-7804, -776),
        Point::new(6789, 7723),
        Point::new(-5107, 5374),
        Point::new(835, -1918),
        Point::new(-6081, -561),
        Point::new(-535, -6860),
        Point::new(2680, -2774),
        Point::new(1076, -5213),
        Point::new(3632, -5693),
        Point::new(2051, 3539),
        Point::new(1794, 3672),
        Point::new(821, -7233),
        Point::new(-151, -7410),
        Point::new(-7452, -3782),
        Point::new(-458, -1583),
        Point::new(5594, -7503),
        Point::new(7414, 8261),
        Point::new(-7065, 8531),
        Point::new(5277, 2794),
        Point::new(100, -7034),
        Point::new(-4959, -222),
        Point::new(3513, 7959),
        Point::new(-361, -4727),
        Point::new(745, -7494),
        Point::new(4841, 2076),
        Point::new(3325, -6982),
        Point::new(3829, -5639),
        Point::new(-2168, 4482),
        Point::new(7124, -3415),
        Point::new(606, -7761),
        Point::new(-2145, 3508),
        Point::new(7138, 7571),
        Point::new(-5915, 1623),
        Point::new(8150, 1532),
        Point::new(7609, -5026),
        Point::new(5420, 1673),
        Point::new(-5786, -2284),
        Point::new(-7257, -3867),
        Point::new(6253, -6783),
        Point::new(4215, 5378),
        Point::new(-6093, 7396),
        Point::new(-5645, -5976),
        Point::new(5119, 1342),
        Point::new(-1882, -7596),
        Point::new(2881, -3271),
        Point::new(-3549, 212),
        Point::new(-1501, -2475),
        Point::new(-6334, -5997),
        Point::new(5671, -7386),
        Point::new(-7790, -2152),
        Point::new(6633, -3292),
        Point::new(1056, 4247),
        Point::new(-3028, 15),
        Point::new(6806, -2413),
        Point::new(941, -5299),
        Point::new(8278, 3554),
        Point::new(5541, 2452),
        Point::new(4486, 7830),
        Point::new(1268, -5525),
        Point::new(4826, -3378),
        Point::new(6734, 7623),
        Point::new(-4862, 6369),
        Point::new(5080, 8004),
        Point::new(-951, -4043),
        Point::new(-5523, -4259),
        Point::new(254, -7275),
        Point::new(6731, 402),
        Point::new(-2500, -2686),
        Point::new(-588, 12),
        Point::new(-4985, -6663),
        Point::new(-4807, -3572),
        Point::new(-6099, -6297),
        Point::new(-247, -7517),
        Point::new(5664, -7343),
        Point::new(-3313, -491),
        Point::new(639, 1576),
        Point::new(928, 7008),
        Point::new(-472, 1164),
        Point::new(5744, 476),
        Point::new(-1726, 7776),
        Point::new(628, -5390),
        Point::new(-5684, -4283),
        Point::new(2917, -7310),
        Point::new(-1, 6835),
        Point::new(-7129, -282),
        Point::new(6158, 5993),
        Point::new(8108, -5268),
        Point::new(-3056, -1923),
        Point::new(-6525, 5864),
        Point::new(-6668, 7756),
        Point::new(-6173, -1360),
        Point::new(-5515, 4842),
        Point::new(5544, 1602),
        Point::new(-6624, 8486),
        Point::new(5204, 7991),
        Point::new(-3535, 115),
        Point::new(5907, -7542),
        Point::new(-6464, -7242),
        Point::new(-8243, -1887),
        Point::new(3671, 1979),
        Point::new(-8057, 7527),
        Point::new(-5115, -3477),
        Point::new(2429, -959),
        Point::new(-5329, 6120),
        Point::new(8429, 28),
        Point::new(-321, 1280),
        Point::new(-1965, 7210),
        Point::new(1004, -574),
        Point::new(2746, -1981),
        Point::new(4386, -3740),
        Point::new(6042, 74),
        Point::new(3867, -6034),
        Point::new(374, 4743),
        Point::new(-1379, -3029),
        Point::new(-313, 1865),
        Point::new(-5490, 613),
        Point::new(-7645, 3469),
        Point::new(6916, 7802),
        Point::new(-138, -6121),
        Point::new(-2735, -6913),
        Point::new(-1902, 2649),
        Point::new(-5602, 2573),
        Point::new(4580, -3699),
        Point::new(1035, 7589),
        Point::new(5786, 245),
        Point::new(-5644, -6621),
        Point::new(3035, -533),
        Point::new(-5943, -1179),
        Point::new(-6823, -4132),
        Point::new(-7130, 1222),
        Point::new(1379, 762),
        Point::new(-2717, 4406),
        Point::new(-2465, 7775),
        Point::new(8548, 2312),
        Point::new(-1012, 1672),
        Point::new(4423, 5917),
        Point::new(4849, 5097),
        Point::new(-6132, 6947),
        Point::new(-5762, 4496),
        Point::new(-3783, -7175),
        Point::new(-1201, -5425),
        Point::new(-7709, 1887),
        Point::new(4411, 5563),
        Point::new(-1888, -4516),
        Point::new(4463, 6000),
        Point::new(8717, -78),
        Point::new(-6546, -5773),
        Point::new(3641, -5630),
        Point::new(3814, -5930),
        Point::new(5632, 364),
        Point::new(-5568, 7497),
        Point::new(8440, -4742),
        Point::new(-3321, -5802),
        Point::new(-9296, -2079),
        Point::new(8029, 1709),
        Point::new(-5394, 1491),
        Point::new(6333, -7398),
        Point::new(3098, -4085),
        Point::new(219, -7479),
        Point::new(242, -7276),
        Point::new(4899, 5714),
        Point::new(13, 2166),
        Point::new(2953, -7381),
        Point::new(-918, 3904),
        Point::new(5988, 996),
        Point::new(-3127, -7142),
        Point::new(-6346, 7747),
        Point::new(4809, -3644),
        Point::new(-2734, -3480),
        Point::new(-3871, -745),
        Point::new(-770, -6453),
        Point::new(517, -3989),
        Point::new(1295, 7141),
        Point::new(2991, -3865),
        Point::new(-5067, 991),
        Point::new(4175, 5766),
        Point::new(2731, 1282),
        Point::new(5930, 2541),
        Point::new(3881, 6036),
        Point::new(3043, -7397),
        Point::new(-7513, 2338),
        Point::new(2794, -1683),
        Point::new(1, 852),
        Point::new(6863, 5545),
        Point::new(6046, -6764),
        Point::new(-5074, 5356),
        Point::new(1466, -1463),
        Point::new(-5555, -3880),
        Point::new(4862, 8206),
        Point::new(7150, -3431),
        Point::new(227, -3970),
        Point::new(2690, -7655),
        Point::new(-5366, -3201),
        Point::new(7701, 3422),
        Point::new(1939, 3978),
        Point::new(-2251, 7423),
        Point::new(6730, -3447),
        Point::new(4556, 8253),
        Point::new(6822, -2607),
        Point::new(4734, -4070),
        Point::new(5168, 1894),
        Point::new(-1585, -7476),
        Point::new(-6899, -4222),
        Point::new(5707, 471),
        Point::new(8213, 1275),
        Point::new(7276, 7671),
        Point::new(4805, 5292),
        Point::new(3923, -5895),
        Point::new(6683, 2452),
        Point::new(4747, 3439),
        Point::new(3671, -5411),
        Point::new(1824, -2073),
        Point::new(7073, -3495),
        Point::new(-4488, -3332),
        Point::new(1354, 2550),
        Point::new(3135, -8105),
        Point::new(-5315, 7272),
        Point::new(3972, 6151),
        Point::new(46, 830),
        Point::new(-4948, 6661),
        Point::new(7375, -3889),
        Point::new(2133, -1904),
        Point::new(3981, -5762),
        Point::new(5322, -7561),
        Point::new(3722, -5913),
        Point::new(-8539, -1871),
        Point::new(5290, 542),
        Point::new(7133, 7786),
        Point::new(-4329, 6810),
        Point::new(6336, -3434),
        Point::new(188, -5343),
        Point::new(3223, -3968),
        Point::new(-2683, -1388),
        Point::new(-7224, -4392),
        Point::new(1203, -5191),
        Point::new(5736, 1816),
        Point::new(-2962, -7171),
        Point::new(-5973, -3680),
        Point::new(1827, 3786),
        Point::new(7671, 2639),
        Point::new(-3284, 5844),
        Point::new(-6894, 1333),
        Point::new(904, 6814),
        Point::new(-3278, -503),
        Point::new(374, -2022),
        Point::new(4881, -3499),
        Point::new(-221, -7562),
        Point::new(-2754, -1593),
        Point::new(8243, -5192),
        Point::new(-6052, -319),
        Point::new(7176, 3649),
        Point::new(-4781, 8133),
        Point::new(6770, 5248),
        Point::new(332, -6960),
        Point::new(4118, -5793),
        Point::new(3368, 634),
        Point::new(2184, 3598),
        Point::new(3046, -7916),
        Point::new(3660, -5898),
        Point::new(7505, -5169),
        Point::new(-5302, -7123),
        Point::new(7502, -36),
        Point::new(6961, 8024),
        Point::new(4815, 8286),
        Point::new(-467, -1936),
        Point::new(2234, 3458),
        Point::new(21, -5864),
        Point::new(3446, -3199),
        Point::new(-6212, 8754),
        Point::new(5315, 665),
        Point::new(3071, 319),
        Point::new(292, 7178),
        Point::new(4259, -3703),
        Point::new(7169, 7812),
        Point::new(3728, -5816),
        Point::new(3736, -5769),
        Point::new(317, 924),
        Point::new(-5262, -560),
        Point::new(8395, 1819),
        Point::new(412, 518),
        Point::new(-4738, 3526),
        Point::new(-4219, 5015),
        Point::new(-3667, 4042),
        Point::new(6834, 5838),
        Point::new(-7977, 7665),
        Point::new(-8144, -4078),
        Point::new(7004, -5501),
        Point::new(-645, -3298),
        Point::new(-7289, -3722),
        Point::new(3308, -5529),
        Point::new(8043, -5314),
        Point::new(-5862, -6823),
        Point::new(5238, 114),
        Point::new(3543, 5867),
        Point::new(3229, -7492),
        Point::new(-2854, -11),
        Point::new(2243, -787),
        Point::new(6064, -7784),
        Point::new(8814, 1047),
        Point::new(-3152, -657),
        Point::new(4487, 5590),
        Point::new(-5893, -6860),
        Point::new(-8123, 4146),
        Point::new(-6609, 3954),
        Point::new(-2193, -1331),
        Point::new(-6247, -5967),
        Point::new(4657, -3735),
        Point::new(-3308, 4643),
        Point::new(4650, 4139),
        Point::new(2324, -1441),
        Point::new(4288, 2815),
        Point::new(6534, 5450),
        Point::new(3128, -4575),
        Point::new(9073, 121),
        Point::new(1036, 7736),
        Point::new(-336, -7584),
        Point::new(-7908, 4752),
        Point::new(5788, 2917),
        Point::new(-7179, 9492),
        Point::new(-6793, -3889),
        Point::new(-5846, -3860),
        Point::new(979, 1920),
        Point::new(-1775, -7192),
        Point::new(1757, -4362),
        Point::new(4679, -3434),
        Point::new(-7525, 4608),
        Point::new(-6062, -5832),
        Point::new(56, 3441),
        Point::new(5641, -370),
        Point::new(-766, -6971),
        Point::new(121, 885),
        Point::new(-7195, -3631),
        Point::new(3527, 7067),
        Point::new(4655, -3445),
        Point::new(5598, 897),
        Point::new(-5896, -6571),
        Point::new(7998, 3730),
        Point::new(4413, -3606),
        Point::new(-3153, 4719),
        Point::new(260, -7406),
        Point::new(7582, 4144),
        Point::new(7445, 4066),
        Point::new(198, -7623),
        Point::new(-1937, -2293),
        Point::new(-4642, 4338),
        Point::new(-2087, 1930),
        Point::new(-2790, -8425),
        Point::new(-5404, -3389),
        Point::new(1314, 6775),
        Point::new(-6336, 162),
        Point::new(-5157, 3250),
        Point::new(57, -1392),
        Point::new(-556, -5691),
        Point::new(2661, -1265),
        Point::new(-5023, -1311),
        Point::new(-7053, 3094),
        Point::new(6617, -3405),
        Point::new(-1035, -4099),
        Point::new(-3784, 3137),
        Point::new(5982, -7456),
        Point::new(3599, -5644),
        Point::new(1629, 3492),
        Point::new(-3146, -208),
        Point::new(-7476, -2304),
        Point::new(-3717, -333),
        Point::new(2006, 3619),
        Point::new(-440, -5986),
        Point::new(8545, -3800),
        Point::new(3841, 8207),
        Point::new(1223, -3063),
        Point::new(-8614, 4168),
        Point::new(3377, -3668),
        Point::new(-4002, 5405),
        Point::new(-5871, 9338),
        Point::new(-5640, 9680),
        Point::new(-3871, 2377),
        Point::new(-6582, 8204),
        Point::new(956, 416),
        Point::new(-3529, 56),
        Point::new(-2854, -2319),
        Point::new(7469, -3255),
        Point::new(-7216, -437),
        Point::new(-6742, -1372),
        Point::new(-6210, -6836),
        Point::new(8578, 1091),
        Point::new(-3424, -574),
        Point::new(3635, -5604),
        Point::new(-2250, -2777),
        Point::new(6079, 136),
        Point::new(-6965, 5583),
        Point::new(4811, 5705),
        Point::new(-5625, -4443),
        Point::new(-370, 2647),
        Point::new(3642, -5706),
        Point::new(8041, 3664),
        Point::new(-5520, -6682),
        Point::new(-2161, 750),
        Point::new(3310, -642),
        Point::new(-5940, -6798),
        Point::new(7264, 7634),
        Point::new(5615, 769),
        Point::new(8360, -5706),
        Point::new(-6353, -4490),
        Point::new(4621, 8627),
        Point::new(2968, 8055),
        Point::new(-5294, -7444),
        Point::new(-3572, -215),
        Point::new(926, 7029),
        Point::new(-823, -4134),
        Point::new(-882, -3267),
        Point::new(4692, 5735),
        Point::new(6323, -7543),
        Point::new(-559, -2322),
        Point::new(-8445, 7122),
        Point::new(-4359, 7375),
        Point::new(-4403, 2035),
        Point::new(394, -7187),
        Point::new(6020, -7409),
        Point::new(-1938, 7234),
        Point::new(-7327, 6021),
        Point::new(-357, -5539),
        Point::new(-2721, 1683),
        Point::new(6464, -5185),
        Point::new(4423, 5850),
        Point::new(5937, 1158),
        Point::new(-2327, -2264),
        Point::new(2987, -8092),
        Point::new(3033, -7499),
        Point::new(-5413, 4491),
        Point::new(7005, -3318),
        Point::new(-7648, 1712),
        Point::new(3997, 8081),
        Point::new(-7859, 4964),
        Point::new(-5158, 5341),
        Point::new(1422, 3734),
        Point::new(5453, 219),
        Point::new(-5783, -6691),
        Point::new(6889, 5192),
        Point::new(-6935, -1208),
        Point::new(-1883, 4518),
        Point::new(6776, 5428),
        Point::new(6393, 5409),
        Point::new(1694, 2163),
        Point::new(331, 900),
        Point::new(-4008, 1259),
        Point::new(7164, 7679),
        Point::new(6922, -3759),
        Point::new(7131, -3804),
        Point::new(-1602, -5019),
        Point::new(8326, 3172),
        Point::new(7100, -3044),
        Point::new(-616, 3755),
        Point::new(-4719, 4910),
        Point::new(5501, 3430),
        Point::new(-7134, 2363),
        Point::new(-7749, -5007),
        Point::new(-4523, -51),
        Point::new(-6064, 94),
        Point::new(6601, -2877),
        Point::new(-3423, -25),
        Point::new(-6392, 2561),
        Point::new(-7761, 5568),
        Point::new(57, -7371),
        Point::new(-373, -6326),
        Point::new(-5220, -3554),
        Point::new(-2558, -2183),
        Point::new(199, -7573),
        Point::new(4175, 8161),
        Point::new(131, -2645),
        Point::new(-1144, -4965),
        Point::new(-2998, -1195),
        Point::new(-6010, -6130),
        Point::new(300, -7257),
        Point::new(-471, -2314),
        Point::new(6886, 7412),
        Point::new(6874, 7689),
        Point::new(-6581, 8015),
        Point::new(-131, -5641),
        Point::new(-3144, -154),
        Point::new(5919, -7290),
        Point::new(485, 1303),
        Point::new(-2455, 4849),
        Point::new(-3479, -1562),
        Point::new(6630, 1565),
        Point::new(-2503, 6408),
        Point::new(-2123, 8011),
        Point::new(6310, 5326),
        Point::new(-3350, -8919),
        Point::new(-2697, -7068),
        Point::new(6626, 5510),
        Point::new(6459, -3112),
        Point::new(7312, 3612),
        Point::new(988, 994),
        Point::new(7249, 5959),
        Point::new(-8148, 8693),
        Point::new(6144, 1364),
        Point::new(5185, 5841),
        Point::new(6297, -2826),
        Point::new(576, 678),
        Point::new(467, -7258),
        Point::new(-5589, -6529),
        Point::new(6000, 282),
        Point::new(1958, -499),
        Point::new(4175, 1846),
        Point::new(6425, -3280),
        Point::new(-4903, 8704),
        Point::new(-6229, -1198),
        Point::new(292, -7050),
        Point::new(4605, 8051),
        Point::new(-3678, 4865),
        Point::new(7033, -3809),
        Point::new(-2086, 7412),
        Point::new(4433, 1546),
        Point::new(6537, -2823),
        Point::new(8469, 1120),
        Point::new(-5226, 4804),
        Point::new(-1942, -5282),
        Point::new(-7318, 4297),
        Point::new(791, 1365),
        Point::new(-7040, -1697),
        Point::new(4680, -3550),
        Point::new(-8762, -3555),
        Point::new(-8423, 5714),
        Point::new(2352, -3631),
        Point::new(-1060, -4578),
        Point::new(-1834, -4688),
        Point::new(-7445, -4984),
        Point::new(2541, -7721),
        Point::new(4044, 5370),
        Point::new(-4357, -457),
        Point::new(-973, 1726),
        Point::new(4199, 3103),
        Point::new(-1730, 7229),
        Point::new(-4485, -3802),
        Point::new(-7187, -5158),
        Point::new(-4204, 5074),
        Point::new(-90, -5322),
        Point::new(660, 417),
        Point::new(8800, 3102),
        Point::new(4959, 5790),
        Point::new(3787, -5629),
        Point::new(-1216, -7386),
        Point::new(-7042, 1594),
        Point::new(-5713, -7063),
        Point::new(-4493, 4168),
        Point::new(7012, 7361),
        Point::new(-2072, -5433),
        Point::new(-550, -6825),
        Point::new(-2530, -3126),
        Point::new(4107, 8514),
        Point::new(3202, -8057),
        Point::new(-3453, 378),
        Point::new(3897, -5582),
        Point::new(-7696, 1645),
        Point::new(-5354, -934),
        Point::new(96, -5109),
        Point::new(-6544, -2858),
        Point::new(4690, -3655),
        Point::new(499, -5863),
        Point::new(659, 7311),
        Point::new(-8743, -6831),
        Point::new(6237, 5324),
        Point::new(-4089, 3416),
        Point::new(4009, -5680),
        Point::new(6012, -7471),
        Point::new(1541, -3180),
        Point::new(6779, -2971),
        Point::new(-7749, 8034),
        Point::new(5457, -7192),
        Point::new(-311, 2982),
        Point::new(4883, -3569),
        Point::new(5080, 5775),
        Point::new(-6198, -140),
        Point::new(-6617, 1610),
        Point::new(-1168, -4652),
        Point::new(4709, -8054),
        Point::new(-3045, 5632),
        Point::new(3880, 8107),
        Point::new(5755, -7521),
        Point::new(-2303, 5316),
        Point::new(6399, 5528),
        Point::new(5550, -7435),
        Point::new(4288, 551),
        Point::new(443, -5495),
        Point::new(3577, 1116),
        Point::new(3009, -865),
        Point::new(-5709, -6232),
        Point::new(3564, -5303),
        Point::new(-4482, -672),
        Point::new(6953, -3099),
        Point::new(2734, -4098),
        Point::new(-7915, -760),
        Point::new(-8794, -5570),
        Point::new(3264, 387),
        Point::new(-1833, -7301),
        Point::new(7539, 8138),
        Point::new(-5258, -7272),
        Point::new(-1453, 1766),
        Point::new(5033, 2183),
        Point::new(-4990, 4987),
        Point::new(-4464, -3604),
        Point::new(-4955, 2109),
        Point::new(7027, -3408),
        Point::new(982, 7091),
        Point::new(6715, 7782),
        Point::new(4548, 7754),
        Point::new(3524, 2864),
        Point::new(-8185, -2337),
        Point::new(3945, 8817),
        Point::new(5888, 3692),
        Point::new(-7968, -4384),
        Point::new(304, -7435),
        Point::new(1262, 1210),
        Point::new(5870, 3787),
        Point::new(-2315, -7470),
        Point::new(-5936, -6518),
        Point::new(928, 1241),
        Point::new(4707, -3238),
        Point::new(4714, 7568),
        Point::new(211, 7318),
        Point::new(6831, 5425),
        Point::new(4328, 6539),
        Point::new(593, -3038),
        Point::new(6510, -3648),
        Point::new(7660, -2885),
        Point::new(5286, 347),
        Point::new(-6283, -6501),
        Point::new(-6463, 1465),
        Point::new(8524, 3182),
        Point::new(4487, 5562),
        Point::new(-5661, -6580),
        Point::new(-1686, 7712),
        Point::new(1776, 3655),
        Point::new(-845, 140),
        Point::new(5604, 2289),
        Point::new(197, -6594),
        Point::new(4220, 5895),
        Point::new(8250, 3533),
        Point::new(6049, 2265),
        Point::new(8170, -5058),
        Point::new(4043, -5732),
        Point::new(5708, 213),
        Point::new(-6128, 8628),
        Point::new(3941, 6938),
        Point::new(-5600, -376),
        Point::new(3895, 2838),
        Point::new(-1969, -4198),
        Point::new(7015, 5632),
        Point::new(49, 1261),
        Point::new(5966, 325),
        Point::new(-4511, 7769),
        Point::new(-5927, -3342),
        Point::new(1696, -2962),
        Point::new(-1800, -4640),
        Point::new(5527, 2540),
        Point::new(3663, -5652),
        Point::new(661, 619),
        Point::new(7744, -788),
        Point::new(-7752, 7626),
        Point::new(3174, -7785),
        Point::new(-894, 2399),
        Point::new(4762, -3586),
        Point::new(3218, -7728),
        Point::new(2764, -7657),
        Point::new(350, -6947),
        Point::new(-1461, 5852),
        Point::new(-3859, -7622),
        Point::new(3992, 7998),
        Point::new(8291, 1346),
        Point::new(-4649, 4972),
        Point::new(640, -7398),
        Point::new(319, -7695),
        Point::new(5447, 1881),
        Point::new(-5621, -6950),
        Point::new(5060, 8401),
        Point::new(240, -7306),
        Point::new(4341, 5909),
        Point::new(-3049, -2216),
        Point::new(340, 1319),
        Point::new(-3207, -2267),
        Point::new(6721, 5372),
        Point::new(-1660, 7475),
        Point::new(6932, 7954),
        Point::new(1535, -2955),
        Point::new(3406, -6021),
        Point::new(1495, 4281),
        Point::new(-6255, -6749),
        Point::new(-3195, -7819),
        Point::new(2162, 3315),
        Point::new(789, 6868),
        Point::new(3895, 8196),
        Point::new(1519, 3870),
        Point::new(-4968, -3437),
        Point::new(345, -6979),
        Point::new(4066, -5634),
        Point::new(3262, -2842),
        Point::new(-2671, 6742),
        Point::new(2096, 3533),
        Point::new(-1959, -7025),
        Point::new(5819, 4192),
        Point::new(3711, -5822),
    ];

    Input {
        n,
        m,
        k,
        stations,
        edges,
        residents,
    }
}
